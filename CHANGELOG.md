...
#### Added
- **`exact_zero_hint` on `--exact` zero-result payloads (#88)** — When `cdidx symbols` / `definition` / `references` / `callers` / `callees` are invoked with `--exact` and return 0 results, cdidx automatically re-runs the same query in substring mode on the server side. If the relaxed query would have matched, the response attaches an `exact_zero_hint` object (CLI JSON + MCP `structuredContent`) with `relaxed_count` and up to 5 distinct `sample_names` from the relaxed hits, plus a short suggestion string. The CLI human-readable path prints a one-line `Hint: --exact found 0 matches, but substring matching would return N (e.g. \`Foo\`, \`FooAsync\`). drop --exact or use the exact indexed casing.` on stderr after the usual "No X found." message. Lets AI agents and humans distinguish three zero-result failure modes: (1) a genuine miss (symbol absent; no hint), (2) a casing mismatch filtered out by `--exact` (especially for non-ASCII identifiers not covered by the NFKC fold from #86), (3) a typo that substring would not have caught either (no hint). Implementation is additive: the relaxed re-query only fires when `exact=true` AND the primary query returned 0 results, so the default substring path pays zero cost. On graph commands the hint is gated behind `reader._hasReferencesTable` so `exact_zero_hint` and the existing degraded-graph notice do not compound. MCP keys stay snake_case — the server's default `PropertyNamingPolicy` is CamelCase, so the hint is built via an explicit `BuildExactZeroHintNode` helper (five call sites) instead of `JsonSerializer.SerializeToNode(dto, _jsonOptions)` to prevent a mixed-shape payload (snake_case outer key + camelCase inner keys) that would break the documented `relaxed_count` / `sample_names` contract for MCP clients — fix surfaced by codex adversarial review. `relaxed_count` is the true relaxed total — dedicated `DbReader.CountSymbolMatches` / `CountReferenceMatches` / `CountCallerMatches` / `CountCalleeMatches` helpers run a `SELECT COUNT(*)` with the same substring filters, and a separate `LIMIT 5` probe collects the sample names. An earlier draft used `SearchSymbols(limit = Math.Max(--limit, 5))` and returned the probe's row count, which over-reported on `--limit 1` (reported 5 when the true total was 11) and under-reported for relaxed sets larger than the caller's limit. Also caught by codex adversarial review. CLI JSON envelope on the exact-zero branch now uses command-specific empty-array keys matching the existing `WriteDegradedGraphZeroResult` convention: `symbols` / `definition` use `"results": []`, graph commands use `"references": []` / `"callers": []` / `"callees": []`. Codex's 3rd adversarial-review pass flagged the envelope was previously missing the marker; the 4th pass flagged that an earlier fix emitted `"results": []` uniformly, inconsistent with the graph commands' degraded envelope. Both are now aligned. Single-probe-first optimization on the exact-miss branch: `ExactZeroHint.FromProbe<T>(probe, countAll, nameOf)` fetches at most `SampleLimit + 1` rows in one pass and only pays the `Count*Matches` query when the probe saturates — typo-style relaxed sets of 5 or fewer siblings are answered with a single substring query instead of two. Codex 4th pass flagged that every exact miss was paying two full LIKE scans. Affected: `src/CodeIndex/Models/QueryResults.cs` (new `ExactZeroHint` DTO + `From` / `ToHumanLine` helpers), `src/CodeIndex/Cli/QueryCommandRunner.cs` (5 commands + shared `EmitExactZeroResult`), `src/CodeIndex/Mcp/McpToolHandlers.cs` (5 MCP tools + shared `BuildExactZeroHintNode`), `tests/CodeIndex.Tests/DbReaderTests.cs`, `tests/CodeIndex.Tests/McpServerTests.cs`, `README.md`. Closes #88.
...
#### 追加
- **`--exact` 0 件時に `exact_zero_hint` を返す (#88)** — `cdidx symbols` / `definition` / `references` / `callers` / `callees` に `--exact` を付けて 0 件だった場合、cdidx は自動で同じクエリを substring モードで再実行する。relaxed 側に hit があった場合は CLI JSON / MCP `structuredContent` に `exact_zero_hint` を付与（`relaxed_count`、distinct サンプル名を最大 5 件、簡単な suggestion）。CLI human 経路は通常の「No X found.」の後に `Hint: --exact found 0 matches, but substring matching would return N (e.g. \`Foo\`, \`FooAsync\`). drop --exact or use the exact indexed casing.` を stderr に 1 行出す。これにより AI エージェントと人間は 0 件の 3 種の原因を切り分けられる: (1) 本物の miss (relaxed も 0 件 → hint 出さず)、(2) `--exact` が casing を取りこぼした（特に #86 の NFKC fold が拾えない非 ASCII 識別子）、(3) substring でも当たらない typo (hint 出さず)。追加実装: relaxed 再検索は `exact=true` かつ 0 件のときだけ走るため、既定 substring 経路は追加コストゼロ。graph 系コマンドでは `reader._hasReferencesTable` で gate し、`exact_zero_hint` と既存の degraded-graph 通知が衝突しないようにする。MCP はサーバー既定の `PropertyNamingPolicy` が CamelCase のため、`JsonSerializer.SerializeToNode(dto, _jsonOptions)` で畳み込むと外キー snake_case + 内キー camelCase の mixed shape になり #88 仕様の `relaxed_count` / `sample_names` 契約を破ってしまう。この問題を回避するため、明示的な `BuildExactZeroHintNode` ヘルパで全 5 ハンドラが snake_case のキーを構築する（codex adversarial review で検出）。`relaxed_count` は真の relaxed 総件数を返すよう、専用の `DbReader.CountSymbolMatches` / `CountReferenceMatches` / `CountCallerMatches` / `CountCalleeMatches` で `SELECT COUNT(*)` を実行し、サンプル名は別途 `LIMIT 5` の probe で取得する。初期実装は `SearchSymbols(limit = Math.Max(--limit, 5))` の結果の行数を `relaxed_count` として使っていたが、`--limit 1` では過大報告（真の件数 11 に対して 5 を返す）、caller limit より大きな relaxed セットでは過少報告になっていた（codex adversarial review で検出）。CLI JSON envelope は exact-zero 分岐で既存の `WriteDegradedGraphZeroResult` と同じコマンド固有の empty-array キーを使う: `symbols` / `definition` は `"results": []`、graph 系は `"references": []` / `"callers": []` / `"callees": []`。Codex 第 3 回で marker 欠如、第 4 回で一律 `"results": []` が graph 系 degraded envelope と不一致であることを指摘されたため揃えた。exact-miss 分岐には single-probe-first 最適化を導入（`ExactZeroHint.FromProbe<T>(probe, countAll, nameOf)`）: `SampleLimit + 1` 件まで 1 回の probe で取り、飽和しない限り `Count*Matches` は呼ばない。typo 系で relaxed 件数が 5 以下に収まる一般ケースは substring クエリ 1 回で済む（codex 第 4 回で 1 exact miss あたり 2 回の LIKE scan を指摘）。対象: `src/CodeIndex/Models/QueryResults.cs`（新規 `ExactZeroHint` DTO + `From` / `ToHumanLine` ヘルパ）、`src/CodeIndex/Cli/QueryCommandRunner.cs`（5 コマンド + 共通 `EmitExactZeroResult`）、`src/CodeIndex/Mcp/McpToolHandlers.cs`（5 MCP ツール + 共通 `BuildExactZeroHintNode`）、`tests/CodeIndex.Tests/DbReaderTests.cs`、`tests/CodeIndex.Tests/McpServerTests.cs`、`README.md`。Closes #88.
