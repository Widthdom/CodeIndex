...
- **Cloud bootstrap prompt now includes a copy-paste fallback for sandbox 403 errors** — Added explicit guidance to ask Codex for unsandboxed network execution only for install/download checks when GitHub access is blocked inside sandbox. Documented in both English and Japanese bootstrap blocks. Affected: `CLOUD_BOOTSTRAP_PROMPT.md`.
...
- **`install.sh` now resolves payload root from the binary path and fails fast on missing runtime assets** — The installer now locates `cdidx` directly inside the extracted archive (supporting both flat and wrapper-directory layouts), errors on ambiguous multi-binary archives, and requires OS-specific runtime assets (`version.json` + `libe_sqlite3.so` on Linux, `version.json` + `libe_sqlite3.dylib` on macOS). If required files are missing, installation aborts with a clear error instead of silently installing a partially broken binary. Affected: `install.sh`.
...
- **Cloud bootstrap prompt にサンドボックス 403 向けコピペ文を追加** — サンドボックス内で GitHub ダウンロードが 403 になる場合に、インストール／ダウンロード確認の部分だけサンドボックス外ネットワーク実行へ切り替えるよう Codex に依頼する定型文を追加。英語ブロック・日本語ブロックの両方に反映。対象: `CLOUD_BOOTSTRAP_PROMPT.md`。
...
- **`install.sh` がバイナリ実体から payload ルートを解決し、必須ランタイム資産欠落時に即時失敗するよう修正** — インストーラは展開後アーカイブ内から `cdidx` の実体パスを直接探索し、フラット配置とラッパーディレクトリ配置の両方を扱う。`cdidx` が複数見つかる曖昧アーカイブは安全のためエラーにする。加えて OS ごとの必須資産（Linux: `version.json` + `libe_sqlite3.so`、macOS: `version.json` + `libe_sqlite3.dylib`）を必須チェックし、欠落時は部分的に壊れたインストールを黙って進めず明確なエラーで中断する。対象: `install.sh`。
