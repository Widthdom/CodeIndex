...
For C# partial types, add `--group-partials` to `definition`, `symbols`, or symbol-mode `inspect` to collapse declarations with the same language, kind, name, and containing-symbol identity into one logical family. The default remains one row per physical declaration. Grouped rows use a stable path/line representative and report the family's physical declaration count in `definition_sites`; grouped count JSON reports both `logical_count` and `physical_count` (plus `physical_file_count`). `impact` uses the same family key automatically. Coordinate-mode `inspect --path ... --line ...` remains a physical lookup and rejects `--group-partials`.
...
| `--group-partials` | `definition`, `symbols`, `inspect` | Collapse matching partial-type declarations into logical families. Off by default. Each grouped result keeps a stable representative and exposes its physical declaration count through `definition_sites`; grouped count JSON includes `logical_count`, `physical_count`, and `physical_file_count`. `inspect` supports this only for symbol queries, not `--path` / `--line` coordinates. |
...
C# の partial type では、`definition`、`symbols`、または symbol mode の `inspect` に `--group-partials` を付けると、language、kind、name、包含シンボルの identity が同じ宣言を1つの論理 family に集約できます。既定は従来どおり物理宣言ごとに1行です。集約行は安定した path / line の代表を使い、family 内の物理宣言数を `definition_sites` で返します。grouped count JSON は `logical_count` と `physical_count`（および `physical_file_count`）を併記します。`impact` は同じ family key を自動的に使います。座標指定の `inspect --path ... --line ...` は物理位置の lookup のままで、`--group-partials` を拒否します。
...
| `--group-partials` | `definition`, `symbols`, `inspect` | 一致した partial type 宣言を論理 family に集約する。既定では無効。各集約結果は安定した代表を保持し、物理宣言数を `definition_sites` で公開する。grouped count JSON は `logical_count`、`physical_count`、`physical_file_count` を返す。`inspect` では symbol query だけに対応し、`--path` / `--line` 座標指定では使えない。 |
