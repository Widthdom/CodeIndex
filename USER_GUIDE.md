...
The internal `.cdidx` data directory is excluded from indexing and workspace-freshness scans. Creating the default `.cdidx/suggestions-codeindex.json` store therefore does not make an otherwise fresh index stale.
...
内部データディレクトリ `.cdidx` は index と workspace freshness の scan 対象外です。そのため、既定の `.cdidx/suggestions-codeindex.json` ストアを作成しても、それ以外が fresh な index は stale になりません。
