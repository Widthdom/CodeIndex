...
- `TestProjectHelper.cs`, `RepositoryTestPaths.cs`, `TestConsoleLock.cs`
  Shared test helpers. Use `TestProjectHelper.DeleteDirectory` / `DeleteFile` for ordinary temp cleanup instead of copying local retry loops; keep local cleanup only for genuinely specialized path shapes such as Windows long-path fixtures.
...
- `TestProjectHelper.cs`、`RepositoryTestPaths.cs`、`TestConsoleLock.cs`
  共有テストヘルパー。通常の temp cleanup はローカルの retry loop をコピーせず、`TestProjectHelper.DeleteDirectory` / `DeleteFile` を使ってください。Windows long-path fixture のように特殊な path shape が必要な場合だけローカル cleanup を残します。
