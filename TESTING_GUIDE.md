...
- Index-finalization readiness coverage keeps reference-cap reads valid inside an active writer transaction and checks mixed C#/VB partial hotspot-family rows with one grouped reader initialization. Preserve both language results when changing readiness SQL; do not replace them with wall-clock thresholds.
...
- LSP telemetry fixtures scope captured activities to an ambient parent trace so unrelated parallel requests cannot enter their assertions. The external-WAL snapshot fixture disposes its readers and writers before resilient project cleanup and must not reintroduce process-wide pool resets.
...
- index-finalization readiness coverage は active writer transaction 中の reference-cap read と、C# / VB の partial hotspot-family rows を1回の grouped reader initialization で検証します。readiness SQL を変更するときは両言語の結果を維持し、wall-clock threshold へ置き換えないでください。
...
- LSP telemetry fixture は capture した activity を ambient parent trace に限定し、無関係な parallel request が assertion に入らないようにする。external-WAL snapshot fixture は resilient な project cleanup 前に reader / writer を dispose し、process-wide pool reset を再導入しないこと。
