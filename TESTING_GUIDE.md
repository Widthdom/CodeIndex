...
- `SymbolExtractor*Tests.cs` と `ReferenceExtractor*Tests.cs`
  ...
  extractor registration snapshot coverage では、同じ language を sequential / concurrent な workspace で使い、後発の user 登録後も公開済み snapshot が維持されることと、一方の workspace reload が他方の extractor や collectible plugin context を変更しないことを検証してください。公開される built-in/user/workspace precedence を固定し、built-in language を上書きできないことも確認します。
