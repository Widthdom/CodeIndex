...
- report / output bundle 用 workspace、import / export archive fixture、configured pattern workspace、audit / metrics / private / global log workspace、legacy-migration database workspace、MCP indexing fixture、suggestion-store fixture、小さな traversal / security fixture を含め、個別のテストファイルで再帰的な一時ディレクトリ cleanup を再実装しないでください。短い呼び出し名で既存テストの読みやすさを保ちたい場合も、ローカル wrapper は `TestProjectHelper.DeleteDirectory` への薄い委譲に留めます。
