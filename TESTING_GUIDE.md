...
- `.github/scripts/configure-windows-test-host.ps1`
  The `dotnet.yml` and `release.yml` Windows lanes share TMP/TEMP pinning and Defender exclusion setup here so both workflows keep the same test-host performance assumptions. The pinned temp root uses a protected current-user ACL so extension fixtures inherit the executable-boundary contract instead of the GitHub runner's shared-temp ACL. Update `CiWorkflowTests` when changing this script or its workflow call contract.
...
- `.github/scripts/configure-windows-test-host.ps1`
  `dotnet.yml` と `release.yml` の Windows lane は、TMP/TEMP 固定と Defender 除外 setup をこのスクリプトで共有します。固定した temp root には current-user 限定の protected ACL を設定し、extension fixture が GitHub runner の shared-temp ACL ではなく executable-boundary contract を継承するようにします。両 workflow の test-host performance 前提を揃えるため、スクリプトまたは workflow からの呼び出し contract を変更するときは `CiWorkflowTests` も更新してください。
- `DbRecoveryTests.cs`
