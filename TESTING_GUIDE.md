...
- `IndexCommandRunner*Tests.cs`, `QueryCommandRunner*Tests.cs`, `ProgramCliTests.cs`, `InstallScriptTests.cs`
  ...
  When a production-runtime test only needs the built CLI to create an indexed fixture, keep that subprocess boundary on the indexing step and run repeated query assertions in-process through the command runner helper instead of spawning one CLI process per lookup.
