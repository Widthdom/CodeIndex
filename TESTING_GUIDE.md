...
- Keep coverage collection, coverage artifact upload, publish, and build artifact upload keyed to the test step's primary-lane outputs (`primary_lane` / `collect_coverage`); do not duplicate the matrix predicate in later artifact steps.
...
- coverage の収集、coverage artifact upload、publish、build artifact upload は test step の primary-lane output (`primary_lane` / `collect_coverage`) に揃え、後続 artifact step で matrix 条件を重複させないでください。
