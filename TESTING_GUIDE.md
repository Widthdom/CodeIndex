...
- `IndexCommandRunner*Tests.cs`, `QueryCommandRunner*Tests.cs`, `ProgramCliTests.cs`, `InstallScriptTests.cs`
  ...
  Batch `--json-summary` coverage keeps side-effect-free dispatch plus successful JSON, NDJSON, and text projection in one seeded fixture when the assertions stay distinct; aggregate output-budget exhaustion remains a separate boundary fixture because of its large payload. Assert `result` / `results` versus raw `stdout` presence explicitly so a typed transport regression cannot pass by reparsing escaped strings in the test.
...
- `IndexCommandRunner*Tests.cs`、`QueryCommandRunner*Tests.cs`、`ProgramCliTests.cs`、`InstallScriptTests.cs`
  ...
  batch `--json-summary` coverage は、assertion を明確に保てる場合、副作用なし dispatch と成功時の JSON / NDJSON / text projection を1つの seeded fixture で共有してください。aggregate output budget の枯渇は大きな payload を使う境界 fixture として分離します。test 側で escaped string を再 parse して typed transport の回帰を見逃さないよう、`result` / `results` と raw `stdout` の有無を明示的に検証してください。
