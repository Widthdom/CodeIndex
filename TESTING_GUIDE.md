...
- Full-scan CLI and MCP no-op coverage treats the single repository-wide reusable-stat snapshot read as a performance contract. Keep assertions for one snapshot read, one stat lookup per candidate, and no content load for unchanged files when changing incremental indexing.
...
- full-scan CLI と MCP の no-op coverage は、リポジトリ全体の reusable-stat snapshot を 1 回だけ読むことを performance contract とします。incremental indexing を変更するときは、snapshot read が 1 回、候補ごとの stat lookup が 1 回、unchanged file の content load が 0 回という assertion を維持してください。
