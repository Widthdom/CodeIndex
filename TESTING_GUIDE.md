...
- `DatabaseTests.cs`, `DatabasePermissionPolicyTests.cs`, `DbReader*Tests.cs`
  ...
  Maintenance-lookup coverage keeps the `files(checksum)` and `file_issues(file_id, kind)` indexes aligned with their predicates, requires `EXPLAIN QUERY PLAN` index `SEARCH` operations for checksum purge, reusable-stat issue probes, and directory/stem rename candidates, and keeps wildcard-bearing, extensionless, and near-match stem semantics in one fixture.
...
- `PreparedCommandCacheTests.DbWriter_WithCache_CSharpStaticInterfaceContractQueriesReuseCacheAndOneWorkspaceRead` keeps the persisted C# contract snapshot and the pending-path contract flag on one physical row pass. Derive both results from that reader; do not restore a second all-contract query for excluded paths.
...
- `DatabaseTests.cs`、`DatabasePermissionPolicyTests.cs`、`DbReader*Tests.cs`
  ...
  maintenance lookup の coverage では `files(checksum)` と `file_issues(file_id, kind)` の index を predicate と同期させ、checksum purge、再利用 stat の issue probe、directory/stem rename 候補が `EXPLAIN QUERY PLAN` で index `SEARCH` を使うことを必須とし、wildcard を含む stem、拡張子なし、近似 stem の意味論を1つの fixture にまとめます。
...
- `PreparedCommandCacheTests.DbWriter_WithCache_CSharpStaticInterfaceContractQueriesReuseCacheAndOneWorkspaceRead` は、永続化済み C# contract snapshot と pending-path contract flag を1回の物理 row passで得ることを固定します。両方を同じ reader から導出し、除外 path 判定のための2回目の全 contract query を戻さないでください。
