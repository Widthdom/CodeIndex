...
- Keep coverage collection and coverage artifact upload keyed to the test step's `collect_coverage` output; do not duplicate the matrix predicate in later artifact steps.
...
- coverage の収集と coverage artifact upload は test step の `collect_coverage` output に揃え、後続 artifact step で matrix 条件を重複させないでください。
