...
- Keep the CI initial test run and its single retry routed through one workflow helper so logger, blame, coverage, and result-directory arguments cannot drift.
...
- CI の初回テスト実行と1回だけの retry は同じ workflow helper 経由にし、logger、blame、coverage、result-directory 引数が drift しないようにしてください。
