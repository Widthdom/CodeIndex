...
- `McpServer*Tests.cs`
  ...
  `RateLimiterTests.cs` uses an injected deterministic clock for bucket-cap saturation and recovery. Assert that bucket count never exceeds the configured cap, cap rejection reports the earliest idle expiry, and advancing exactly to the advertised retry boundary allows a legitimate new bucket. MCP integration coverage keeps known-tool partitions independent, drives missing/non-string/empty/oversized/case-variant/unknown names plus invalid arguments through the pre-validation normalizer, and verifies that unknown `batch_query` slot names share the same bounded partition.
...
- `McpServer*Tests.cs`
  ...
  `RateLimiterTests.cs` は bucket 上限の飽和と回復に注入した決定論的 clock を使います。bucket 数が設定上限を超えないこと、上限拒否が最も早い idle expiry を返すこと、通知された retry 境界まで正確に進めると正規の新規 bucket を作成できることを検証してください。MCP integration coverage では既知 tool の partition が独立することを保ち、missing/non-string/empty/oversized/case-variant/unknown 名と invalid argument を pre-validation normalizer に通し、unknown な `batch_query` slot 名も同じ bounded partition を共有することを検証してください。
