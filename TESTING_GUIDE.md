...
- Incremental FTS maintenance coverage shares the 25-run optimize threshold across full scan, scoped update, and MCP indexing. Assert that the first mutating incremental run records one write without invoking the optimize hook, the threshold run invokes it once and resets the counter, and fresh/rebuild bulk-load coverage still optimizes immediately.
...
- reference-graph refresh coverage は、incremental full scan、scoped `--files` update、MCP indexing を横断する graph-neutral indexing を performance contract とします。symbol/reference を持たない新規・変更 source file では refresh 0 回を維持し、既存または新規の graph identity 行が変化する場合は batch 全体で refresh 1 回を assertion してください。
...
- incremental FTS maintenance coverage は、full scan、scoped update、MCP indexing で25 run の optimize threshold を共有します。最初の変更付き incremental run は write を1回記録して optimize hook を呼ばず、threshold 到達 run は hook を1回呼んで counter をresetし、fresh/rebuild bulk-load coverage は引き続き即時 optimize することを assertion してください。
