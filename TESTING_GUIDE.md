...
- `HotspotReferenceAggregateTests.cs`
  Regression coverage for the maintained per-file hotspot aggregate, including legacy transactional backfill and representative limited file/name queries. The query-plan assertions require `hotspot_reference_counts`, reject raw `symbol_references` scans, and use the shared broad deterministic timeout instead of a benchmark-grade threshold.
...
- `HotspotReferenceAggregateTests.cs`
  file 単位 maintained hotspot aggregate の回帰 coverage です。legacy database の transactional backfill と、代表的な limit 付き file/name query を含みます。query-plan assertion は `hotspot_reference_counts` の利用を必須とし、raw `symbol_references` scan を拒否します。benchmark 用の厳しい閾値ではなく、共有の広い deterministic timeout を使います。
- `.github/scripts/run-dotnet-tests.ps1`
