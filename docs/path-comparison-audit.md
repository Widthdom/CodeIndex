...
| Extension and filename conventions | `FileIndexer`, `FileIndexer.FileNameLanguages`, `FileIndexer.DefaultExclusions`, `SymbolExtractor.TypeScriptPathAliases` | Keep extensions, generated-file markers, and cross-platform convention tokens case-insensitive. Exact filenames and filename-prefix patterns used for language detection follow the probed filesystem `path_case_sensitive` policy so distinct names are not collapsed on case-sensitive filesystems. |
...
| 拡張子とファイル名の慣習 | `FileIndexer`, `FileIndexer.FileNameLanguages`, `FileIndexer.DefaultExclusions`, `SymbolExtractor.TypeScriptPathAliases` | 拡張子、generated-file marker、cross-platform convention token は case-insensitive のままにする。言語判定に使う完全一致ファイル名と filename-prefix pattern は、case-sensitive filesystem 上の別名を畳み込まないよう、実測した `path_case_sensitive` ポリシーに従う。 |
