...
| `McpServer` | `_concurrencyGate` | The server owns the gate as the effective upper bound for concurrently executing stdio and request-context-aware transport requests. Single requests and independent JSON-RPC batch items acquire slots from the same gate only at dispatch, so a batch frame cannot self-deadlock and a base transport at `maxConcurrency: 1` does not double-acquire an outer permit. Execution timeout begins after acquisition, and a timed-out cancellation-insensitive action retains its lease until true completion. Batch initialize/session mutations and repeated request IDs form input-order fences, while cancellation controls bypass the gate. The managed gate remains undisposed because bounded transport teardown may leave a late releaser. | Signal-driven stdio/HTTP/base/batch overlap, cap, ordering, queued cancellation, timeout-lease, and teardown tests. |
| `McpServer` | local `admissionGate`, `_activeRequests` | Each concurrent frame loop separately bounds accepted ordinary frames at `MaxConcurrency + 64` by default. Admission is non-blocking so cancellation/client-response controls remain readable; overflow requests receive retry-safe `-32003` / `server_busy`. Ordinary single-request IDs enter `_activeRequests` before protocol or execution waits. | Admission overflow and more-than-64 queued-cancellation tests. |
| `McpServer` | `_queuedBatchRequests` | The batch preregistration pass durably owns a linked cancellation source for each unique ordinary batch ID until dispatch. Dispatch links that source into the request CTS before moving the ID to `_activeRequests`; authentication, shape-validation, duplicate, and other pre-dispatch returns release an unclaimed registration. Queued batch cancellation is therefore independent of the short tombstone cap and TTL without exposing unauthenticated IDs as active requests. | Tombstone-cap/TTL cancellation, pre-dispatch cleanup, same-batch, and authenticated HTTP tests. |
| `McpServer` | `_initializeStateGate`, immutable `InitializeSessionState`, protocol barrier tasks | Writers use the lock to publish client identity, capabilities, roots, and caller metadata by replacing one immutable state reference; readers capture that reference once so a request cannot mix fields across re-initialization. In a batch, the coordinator exposes a successful initialize draft through a frame-local immutable view only after adopting that exact item response, so later ordered segments see the new session while global state remains unchanged. Each segment captures its own generation, so timeout-draining work from an earlier segment remains pinned to the earlier session. Global commit occurs only after the exact frame response is serialized. Initialize and other session-mutating frames form receive-order barriers: they wait for previously accepted work, while later normal requests wait through deferred state commit and transport write without blocking cancellation or client-response frames. | Initialization-order, coherent-session-state, and concurrent request tests. |
...
| `HttpMcpTransport` | `_queueSlots` | The transport owns queue capacity. HTTP handlers acquire a slot before `TryWrite`; `ReadFrameAsync` releases the slot after dequeuing. Bearer authorization precedes out-of-band handling. Cross-frame cancellation controls are extracted once before admission, while a control targeting an ID in the same batch remains in the raw batch for durable server preregistration. Full queues reject remaining requests instead of blocking handlers indefinitely. | HTTP MCP queue-limit, auth-order, and mixed-batch cancellation tests. |
...
| `McpServer` | `_concurrencyGate` | server が stdio と request-context 対応 transport request の実効的な同時実行上限として所有します。single request と JSON-RPC batch の独立 item は dispatch 時だけ同じ gate から slot を取得するため、batch frame は自己 deadlock せず、base transport も `maxConcurrency: 1` で outer permit を二重取得しません。execution timeout は取得後に開始し、timeout 後も cancellation を無視する action は実完了まで lease を保持します。batch 内の initialize/session mutation と重複 request ID は入力順 fence になり、cancellation control は gate を bypass します。bounded transport teardown 後に late releaser が残り得るため managed gate は dispose しません。 | signal-driven な stdio/HTTP/base/batch overlap、上限制御、順序、queued cancellation、timeout lease、teardown tests。 |
| `McpServer` | local `admissionGate`、`_activeRequests` | concurrent frame loop ごとに通常 accepted frame を既定で `MaxConcurrency + 64` に制限します。admission は non-blocking のため cancellation/client-response control を読み続け、超過 request には retry-safe な `-32003` / `server_busy` を返します。通常の single-request ID は protocol/execution 待機前に `_activeRequests` へ入ります。 | admission overflow と 64 件超 queued-cancellation tests。 |
| `McpServer` | `_queuedBatchRequests` | batch の事前登録 pass から dispatch まで、通常 batch item の各 unique ID に対応する linked cancellation source を durable に所有します。dispatch は source を request CTS に link してから ID を `_activeRequests` へ移し、認証、shape validation、duplicate などで dispatch 前に return した場合は未 claim の登録を解放します。未認証 ID を active request として公開せず、queued batch cancellation を短命 tombstone の件数上限と TTL から独立させます。 | tombstone cap/TTL cancellation、dispatch 前 cleanup、same-batch、認証付き HTTP tests。 |
| `McpServer` | `_initializeStateGate`、immutable `InitializeSessionState`、protocol barrier task | writer は lock 内で 1 つの immutable state reference を置き換え、client identity、capabilities、roots、caller metadata を一括 publish します。reader は reference を一度だけ取得するため、re-initialize 前後の field が同じ request 内で混在しません。batch では coordinator が採用した initialize item の成功 response と draft が完全一致した後だけ frame-local な immutable view を公開するため、後続の順序 segment は新しい session を参照しますが、global state は変わりません。各 segment は固有 generation を保持するため、timeout 後も drain 中の先行 work は旧 session に固定されます。global commit は対応する frame response の serialize 成功後だけ行います。initialize など session を変更する frame は受信順 barrier となり、先に受理した処理を待ち、後続通常 request は deferred state commit と transport write の完了まで barrier を待ちます。cancellation/client-response frame は block しません。 | initialization 順序、一貫した session state、並行 request tests。 |
...
| `HttpMcpTransport` | `_queueSlots` | transport が queue capacity を所有します。HTTP handler は `TryWrite` 前に slot を取得し、`ReadFrameAsync` が dequeue 後に slot を返します。bearer 認証は out-of-band handling より先です。cross-frame cancellation control は admission 前に一度だけ抽出し、same-batch ID を対象にする control は durable な server 事前登録のため raw batch に残します。queue full 時は残り request を拒否し、handler を無期限 block しません。 | HTTP MCP queue-limit、auth-order、mixed-batch cancellation tests。 |
