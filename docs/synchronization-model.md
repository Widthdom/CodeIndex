...
| `McpServer` | local `writeGate`, `normalFrameGate` | One frame loop owns these gates. `writeGate` serializes transport writes; `normalFrameGate` preserves normal request frame ordering while allowing cancellation and response frames to bypass that lane. Clean stdio EOF drains accepted request tasks to completion; cancellation paths retain bounded cleanup, so gates are disposed only when all tasks have completed. | MCP frame-loop ordering and drain tests. |
...
| `McpServer` | local `writeGate`, `normalFrameGate` | 1 つの frame loop が所有します。`writeGate` は transport write を直列化し、`normalFrameGate` は通常 request frame の順序を保ちながら cancellation/response frame を別 lane で処理できるようにします。正常な stdio EOF は受理済み request task を完了まで drain し、cancellation 経路は bounded cleanup を維持するため、全 task 完了時だけ gate を dispose します。 | MCP frame-loop ordering と drain tests。 |
