  ...
  Indexer/
    ...
    References/
      ...
      Support/
        ...
        TrailingLambdaReferenceExtractor.cs — Shared trailing lambda/block call helpers for Kotlin, Scala, and Swift
...
|---|---|---|---|---|---|---|---|---|:---:|
| Python | def, async def, def NAME[T](...), async def NAME[T](...) | class | -- | -- | -- | @property | -- | from/import, type NAME = ... | yes |
| JavaScript | function, top-level generators and async generators (`function*`, `async function*`), arrow, methods (including same-line keyword/modifier-named methods, default arguments, computed names, `#private`, generator, `async *`, class-field arrow properties such as `handleClick = () => {}`), object-literal methods / accessors / generator shorthand (`{ foo() {}, get foo() {}, set foo(v) {}, *bar() {}, async baz() {} }`, including `module.exports = { ... }`), qualified assignments such as `Vehicle.prototype.start = function () {}`, `Foo.Bar = class {}`, and `Vehicle.VERSION = "1.0"`, named function expressions such as `var add = function add() {}`, direct CommonJS named export bindings such as `module.exports.foo = function () {}` / `module.exports.bar = () => {}`, including same-line and multiline parenthesized wrappers while keeping identifier prefixes like `functionCall()` / `classyThing` as property values, PascalCase HOC-wrapped / call-result / tagged-template bindings — RHS restricted to known HOC prefixes (`React.memo(`/`React.forwardRef(`/`React.lazy(` only; bare `React.` is NOT accepted so non-HOC React APIs like `React.createContext(...)` / `React.useCallback(...)` / `React.useMemo(...)` / `React.createRef(...)` do NOT emit phantom `function` symbols — `styled.`/`styled(`/`styled\``, bare `connect(`/`memo(`/`forwardRef(`/`lazy(`/`observer(`/`with<PascalCase>(`) so `const Config = loadConfig();`, `const ENV = ...;`, and `const Widget = class extends ...` do NOT emit phantom `function` symbols (`= class` is owned by the class-expression pass); a post-match gate (`ShouldSkipJavaScriptTypeScriptStyledFactoryCandidate`) additionally rejects `styled.xxx;` / `styled(x);` factory-capture / plain-call shapes whose raw source statement (from the match end to the next `;` or next statement) contains no real-code backtick — the scan is comment-/string-aware (skips `//` line comments, `/* ... */` block comments including block comments that span across physical lines, and `'...'` / `"..."` string literals) so backticks and `;` characters that only live inside comments or plain strings cannot steer the decision (an unrelated tagged template on a later statement of the same physical line does not keep the binding alive because the scan stops at the next real `;`), and the scan is multi-line aware within a bounded lookahead window so Prettier-style bindings whose tagged-template backtick sits on the physical line after `styled.div` / `styled(Component)` still survive (on each continuation line the first non-whitespace, non-comment character must be one of backtick / `.` — a valid tagged-template continuation starter; anything else, including identifier-start expression statements such as `foo(\`x\`)` or top-level `await foo(\`x\`)`, declaration statements such as `const Y = 5;`, and JSX-element / TS-cast starts such as `<Foo>{...}`, is treated as ASI-inserted statement termination and causes the gate to reject so `const X = styled.div\nfoo(\`x\`)`, `const X = styled.div\nawait foo(\`x\`)`, `const X = styled.div\nconst Y = 5;`, and `const X = styled.div\n<Foo>{...}` all stay rejected — `<` is intentionally NOT whitelisted because styled-components generics always appear before the backtick on the same expression, never on a new line); within the scan the gate also tracks paren / bracket / angle / brace depth so a backtick inside a nested expression (e.g. `.attrs({ ... })`) does not count as the tag head, and a depth-0 operator character (`+`, `-`, `*`, `%`, `?`, `!`, `&`, `\|`, `^`, `=`, `,`, `:`) between the match end and the first backtick terminates the tag-head chain and forces rejection so `const X = styled.div + \`not a tag\`` and `const Y = styled(C) + \`also not\`` do NOT emit phantom `function` symbols; when the regex match already consumed an opening paren (`styled(`, `memo(`, `connect(`, etc.) the depth counter starts at -1 so the upcoming matching `)` restores it to 0 rather than going further negative, keeping the Prettier-style multi-line `styled(Component)\n\`...\`` shape accepted; the gate also walks across the entire template body (including `${...}` substitutions and raw line boundaries with `\\` escape handling) when it sees the depth-0 tag-head backtick, sets a `tagHeadConsumed` flag, and continues scanning so a depth-0 operator character appearing **after** the closing backtick still rejects (`const NotStyledPlusTheme = styled.div\`color: red\` + theme;` does NOT emit a phantom symbol) while a depth-0 `;` after the closing backtick accepts the binding cleanly; the lookahead window walks 32 physical lines (raised from 8) so Prettier-formatted bindings whose `.attrs((props) => ({ ... }))` argument object spans more than 10 lines before the backtick still resolve to the trailing tagged template; continuation-line first-character checking is skipped while `depth > 0` so lines 2+ of a multi-line nested-expression body (e.g. `field1: props.value1,` inside `.attrs((props) => ({ ... }))`) are recognised as expression continuation rather than ASI-inserted new statements; and the arrow-function token `=>` is recognised as a single unit so `>` does not falsely decrement depth and expose subsequent depth-0 operator characters inside arrow bodies (`(props) => prop1 ? a : b`) to spurious rejection; so only the tagged-template shape `styled.div\`...\`` / `styled(Component)\`...\`` survives for the `styled.` / `styled(` prefixes; declaration-only `BodyStyle.None`; lowercase bindings also skipped | class, export default class, same-line sibling/statement-prefixed public classes, class expressions, multiline/parenthesized/CommonJS class exports | -- | -- | -- | property-like export surface symbols for ESM re-exports (`export * from`, `export * as ns from`, `export { foo as bar } from`, including commented / multiline / minified / `with {}` / `assert {}` forms such as multiline `export *` / `export * as ns` and minified `export*from` / `export{foo as bar}from`) and exported object-literal alias/shorthand properties such as `module.exports = { foo: inner }` / `module.exports = { foo, bar }` | -- | import...from | yes |
| TypeScript | function, top-level generators and async generators (`function*`, `async function*`), arrow, methods (including generic and same-line object/conditional/function-return methods, default arguments, computed names, `#private`, generator, `async *`, class-field arrow properties such as `handleClick = (): void => {}` with parsed return type), object-literal methods / accessors / generator shorthand (`{ foo(): T {}, get foo(): T {}, set foo(v: T) {}, *bar(): Generator<T> {} }`), direct CommonJS named export bindings such as `module.exports.foo = function () {}` / `module.exports.bar = () => {}` / `module.exports.fn = <T>(x: T) => x`, including same-line and multiline parenthesized wrappers, multiline / constrained / async generic-arrow headers, and prefix-safe function/class discrimination while keeping identifier prefixes like `functionCall()` / `classyThing` as property values, PascalCase HOC-wrapped / call-result / tagged-template bindings with optional `: <type>` annotation — same narrow HOC-prefix allowlist as JavaScript (strict `React.memo(` / `React.forwardRef(` / `React.lazy(`, not bare `React.`, so `const Theme: React.Context<T> = React.createContext(...)` and other non-HOC React API calls do NOT emit phantom `function` symbols) and the same styled-factory post-match gate (`ShouldSkipJavaScriptTypeScriptStyledFactoryCandidate`) that limits `styled.` / `styled(` to the tagged-template shape on the same raw source statement (scans from the match end to the next `;` / next statement with basic JS/TS lexer awareness — `//` line comments, `/* ... */` block comments including block comments that span across physical lines, and `'...'` / `"..."` string literals are skipped, so only real-code backticks and `;` characters contribute to the decision, and an unrelated tagged template later on the same physical line does not re-open the gate; the scan walks across physical lines within a bounded lookahead window so Prettier-style multi-line tagged templates still survive, and on each continuation line the first non-whitespace non-comment character must be a valid tagged-template continuation starter — backtick / `.` — anything else such as an identifier-start expression statement (`foo(\`x\`)`), top-level `await`, a declaration keyword (`const Y = 5;`), or a JSX-element / TS-cast start (`<Foo>{...}`) is treated as ASI-inserted statement termination and causes the gate to reject (`<` is intentionally NOT whitelisted because styled-components generics always appear before the backtick on the same expression, never on a new line); within the scan the gate also tracks paren / bracket / angle / brace depth so a backtick inside a nested expression (e.g. `.attrs({ ... })`) does not count as the tag head, and a depth-0 operator character (`+`, `-`, `*`, `%`, `?`, `!`, `&`, `\|`, `^`, `=`, `,`, `:`) between the match end and the first backtick terminates the tag-head chain and forces rejection so `const X = styled.div + \`not a tag\``, `const Y = styled(C) + \`also not\``, and `const Z: StyledComponent<'div'> = styled.div + \`still not\`` do NOT emit phantom `function` symbols; when the regex match already consumed an opening paren (`styled(`, `memo(`, `connect(`, etc.) the depth counter starts at -1 so the upcoming matching `)` restores it to 0 rather than going further negative, keeping the Prettier-style multi-line `styled(Component)\n\`...\`` shape accepted; the gate also walks across the entire template body (including `${...}` substitutions and raw line boundaries with `\\` escape handling) when it sees the depth-0 tag-head backtick, sets a `tagHeadConsumed` flag, and continues scanning so a depth-0 operator character appearing **after** the closing backtick still rejects (`const AnnotatedNotStyledPlusTheme: StyledComponent<'div'> = styled.div\`color: red\` + theme;` does NOT emit a phantom symbol) while a depth-0 `;` after the closing backtick accepts the binding cleanly; the lookahead window walks 32 physical lines (raised from 8) so Prettier-formatted bindings whose `.attrs((props) => ({ ... }))` argument object spans more than 10 lines before the backtick still resolve to the trailing tagged template; continuation-line first-character checking is skipped while `depth > 0` so lines 2+ of a multi-line nested-expression body (e.g. `field1: props.value1,` inside `.attrs((props) => ({ ... }))`) are recognised as expression continuation rather than ASI-inserted new statements; and the arrow-function token `=>` is recognised as a single unit so `>` does not falsely decrement depth and expose subsequent depth-0 operator characters inside arrow bodies (`(props) => prop1 ? a : b`) to spurious rejection); unlike the JavaScript row, the TypeScript row also accepts an optional `<TypeArgs>` token directly on the HOC call (`React.forwardRef<HTMLDivElement, Props>(...)`, `React.memo<Props>(...)`, `connect<State, Dispatch>(...)`, `with<Pascal><T>(...)`) with up to three levels of nested angle brackets (`React.memo<Record<string, Map<string, Props>>>(...)`) and one level of nested parenthesised segments inside a generic argument (`React.memo<(cb: (x: string) => void) => void>(...)`) via a shared `TypeScriptOptionalHocTypeArgsPattern` constant whose alternation branches start with disjoint character classes so catastrophic backtracking cannot occur; the JavaScript row intentionally does NOT carry this optional token because JavaScript has no generic syntax and `const Result = memo < Props > (Component);` is a chained comparison / call expression that must not produce a phantom HOC binding; the arrow-function row also accepts the same optional annotation so typed arrow bindings whose type contains `=>` (`const Callback: (x: number) => number = (x) => x + 1`) still match the arrow row with `BodyStyle.Brace` instead of being shadowed by the HOC row; typed HOC bindings such as `const Connected: React.ComponentType<Props> = connect(...)(Component)` and `const Styled: StyledComponent<'div', Theme> = styled.div\`...\`` still capture via the HOC row; declaration-only `BodyStyle.None` on the HOC row; lowercase bindings also skipped | class, export default class, anonymous default `abstract class`, `export = class`, same-line sibling/statement-prefixed public classes, class expressions, multiline/parenthesized class expressions, type | -- | interface | enum, const enum | property-like export surface symbols for ESM/TS re-exports (`export * from`, `export * as ns from`, `export { foo as bar } from`, `export type { User } from`, `export type * from`, `export type * as ns from`, including commented / multiline / minified / `with {}` / `assert {}` forms such as multiline `export *` / `export * as ns` and minified `export*from` / `export{foo as bar}from`) and exported object-literal alias/shorthand properties such as `module.exports = { foo: inner }` / `module.exports = { foo, bar }` | -- | import...from | yes |
| C# | methods, ctors, explicit-interface impls (including qualifiers with multi-argument generics like `IMap<string, int>.GetCount`, nullable type arguments like `IFoo<string?>.NullableArg`, and array type arguments like `IFoo<int[]>.ArrayArg`), and indexers (including `ref` / `ref readonly` returns, pointer / function-pointer returns such as `int*` / `void**` / `delegate*<int, int>` / `int*[]`, and tuple returns with trailing `[]` / `?` / `[,]` / `[][]` suffixes such as `(int, int)[]` / `(int, int)?` / `(int, int)[][]` / `(int, int)[,]`; guards named-argument labels only before qualified call expressions; allows `global::` / alias-qualified return types and spaced generic type tokens; attribute-stripper blanks out multi-section attributes such as `[Obsolete, Conditional("DEBUG")]` and `[Fact, Trait("cat","io")]` so trailing attribute names are not leaked as phantom `function` symbols; LINQ query-expression contextual keywords such as `from` / `where` / `select` / `orderby` / `group` / `join` / `let` / `into` / `on` / `equals` / `ascending` / `descending` / `by` are excluded from the return-type position so continuation lines like `where Validator.Check(x)` / `select Mapper.Convert(x)` / `orderby Math.Abs(x)` do not emit phantom `function` symbols for the qualified member name; modifier order is free, so visibility may appear at any position in the modifier sequence on every C# row — type declarations (`abstract public class`, `sealed public class`, `readonly public struct`, `ref public struct`, `partial public interface`, `abstract public record class`), `const` fields (`new public const int X = 1;`, `public new const int X = 1;`), `static readonly` fields (`readonly public static int E = 6;`), methods (`static public int F() => 0;`), properties (`static public int P { get; set; }`), indexers (`static public int this[int i] => 0;`), events, delegates, operator / conversion-operator overloads, constructors (`unsafe public S(int* p) {}`, `extern public S(int x);`), and static constructors (`unsafe static S()`); wrapped-modifier constructors where the leading modifier (`static` / `public` / `private` / `protected` / `internal` / `unsafe` / `extern`) sits on its own physical line before the `Name(...)` header — including attribute-preceded shapes such as `[System.Obsolete]\nstatic\nD() { ... }` — are recovered via a wrapped-modifier retry so the symbol is emitted at the identifier line with the modifier chain restored in `signature` — `unsafe` / `extern` are also accepted as free-order modifiers on the property / indexer / event / constructor / static-constructor rows, `static` / `readonly` may be interleaved with `new` in any order (e.g. `readonly new static`, `new readonly static`), C# inheritance modifiers `virtual` / `override` / `abstract` / `sealed` / `new` are accepted as free-order modifiers on event declarations (`abstract public event E;`, `sealed public override event E;`), and the `file` file-scoped type modifier plus nested `new` are accepted on `interface` and `delegate` rows (`file interface I {}`, `file delegate int D(int x);`)), operators stored as `operator +` / `operator checked +` / `operator checked -` (including unary, binary, and other C# 11 user-defined checked operators), conversion operators stored as `implicit operator decimal` / `explicit operator Money` / `explicit operator checked int` (including `unsafe` / `extern` forms and function-pointer target types), indexers normalized to `Item`, const, static readonly, enum members, #region, finalizers | class, record (wrapped headers preserve base list and `where` clauses in `symbols.signature`, including C# 12 primary constructor parameter lists) | struct, record struct, ref struct (wrapped headers preserve base list and `where` clauses in signature) | interface (wrapped headers preserve base list and `where` clauses in signature) | enum (wrapped headers preserve `: underlyingType` in signature) | property, partial property, expression-bodied, `ref` / `ref readonly` properties, pointer properties, tuple-array / nullable-tuple properties, explicit-interface property implementations (both brace-body `int IThing.Value { get; set; }` and expression-body `string IThing.Name => "x";`, including generic interface qualifiers like `IBucket<int>.Items` and multi-argument generic qualifiers like `IMap<string, int>.PairCount`), record primary components, plain fields (instance, readonly, volatile, plain static, with or without initializer, multi-line declarations that wrap before `;` — including parenthesized / constructor-call initializers such as `= new(\n    () => 42);` and object / collection initializers such as `= new() { ... };` / `= new Dictionary<...> { ... };` — declarator lists such as `int _x, _y;` — one symbol per declarator — and function-pointer fields such as `delegate*<int, void> Callback;` / `delegate* unmanaged[Cdecl]<int, int> _op;`) | event, delegate (including spaced generic type tokens and pointer returns) | using, using alias, extern alias | yes |
| C# | methods, ctors, explicit-interface impls (including qualifiers with multi-argument generics like `IMap<string, int>.GetCount`, nullable type arguments like `IFoo<string?>.NullableArg`, and array type arguments like `IFoo<int[]>.ArrayArg`), and indexers (including `ref` / `ref readonly` returns, pointer / function-pointer returns such as `int*` / `void**` / `delegate*<int, int>` / `int*[]`, and tuple returns with trailing `[]` / `?` / `[,]` / `[][]` suffixes such as `(int, int)[]` / `(int, int)?` / `(int, int)[][]` / `(int, int)[,]`; guards named-argument labels only before qualified call expressions; allows `global::` / alias-qualified return types and spaced generic type tokens; attribute-stripper blanks out multi-section attributes such as `[Obsolete, Conditional("DEBUG")]` and `[Fact, Trait("cat","io")]` so trailing attribute names are not leaked as phantom `function` symbols; LINQ query-expression contextual keywords such as `from` / `where` / `select` / `orderby` / `group` / `join` / `let` / `into` / `on` / `equals` / `ascending` / `descending` / `by` are excluded from the return-type position so continuation lines like `where Validator.Check(x)` / `select Mapper.Convert(x)` / `orderby Math.Abs(x)` do not emit phantom `function` symbols for the qualified member name; modifier order is free, so visibility may appear at any position in the modifier sequence on every C# row — type declarations (`abstract public class`, `sealed public class`, `readonly public struct`, `ref public struct`, `partial public interface`, `abstract public record class`), `const` fields (`new public const int X = 1;`, `public new const int X = 1;`), `static readonly` fields (`readonly public static int E = 6;`), methods (`static public int F() => 0;`), properties (`static public int P { get; set; }`), indexers (`static public int this[int i] => 0;`), events, delegates, operator / conversion-operator overloads, constructors (`unsafe public S(int* p) {}`, `extern public S(int x);`, with a positional negative lookahead that rejects lines whose matching `)` is followed by an identifier + `{` / `(` / `=>` (with optional `?` / `[]` / `[,]` / whitespaced tuple suffixes such as `) []` / `) ?` / `)  ?` in between, factored into a shared `CSharpTupleSuffixPattern` constant consumed by both `CSharpTypePattern` and the ctor lookahead so the two stay in lock-step) — the exact shape of a property or expression-bodied method with a modifier keyword, so contextual / reserved keywords like `partial` / `required` / `readonly` / `async` / `sealed` / `virtual` / `override` / `abstract` / `new` / `file` / `static` cannot be captured as the constructor name when the method / property regex upstream fails — e.g. `public required (int, int) R1 { get; init; }` and `public required (int, int) [] R4 { get; init; }` previously emitted a phantom `function required` row; multi-line ctor signatures, `extern` ctors ending in `;`, expression-bodied ctors, `: base(...)` / `: this(...)` initializers, and tuple-parameter ctors are all unaffected), and static constructors (`unsafe static S()`) — `unsafe` / `extern` are also accepted as free-order modifiers on the property / indexer / event / constructor / static-constructor rows, `static` / `readonly` may be interleaved with `new` in any order (e.g. `readonly new static`, `new readonly static`), C# inheritance modifiers `virtual` / `override` / `abstract` / `sealed` / `new` are accepted as free-order modifiers on event declarations (`abstract public event E;`, `sealed public override event E;`), and the `file` file-scoped type modifier plus nested `new` are accepted on `interface` and `delegate` rows (`file interface I {}`, `file delegate int D(int x);`)), operators stored as `operator +` / `operator checked +` / `operator checked -` (including unary, binary, and other C# 11 user-defined checked operators), conversion operators stored as `implicit operator decimal` / `explicit operator Money` / `explicit operator checked int` (including `unsafe` / `extern` forms and function-pointer target types), indexers normalized to `Item`, const, static readonly, enum members, #region, finalizers | class, record (wrapped headers preserve base list and `where` clauses in `symbols.signature`, including C# 12 primary constructor parameter lists) | struct, record struct, ref struct (wrapped headers preserve base list and `where` clauses in signature) | interface (wrapped headers preserve base list and `where` clauses in signature) | enum (wrapped headers preserve `: underlyingType` in signature) | property, partial property, expression-bodied, `ref` / `ref readonly` properties, pointer properties, tuple-array / nullable-tuple properties, explicit-interface property implementations (both brace-body `int IThing.Value { get; set; }` and expression-body `string IThing.Name => "x";`, including generic interface qualifiers like `IBucket<int>.Items` and multi-argument generic qualifiers like `IMap<string, int>.PairCount`), record primary components, plain fields (instance, readonly, volatile, plain static, with or without initializer, multi-line declarations that wrap before `;` — including parenthesized / constructor-call initializers such as `= new(\n    () => 42);` and object / collection initializers such as `= new() { ... };` / `= new Dictionary<...> { ... };` — declarator lists such as `int _x, _y;` — one symbol per declarator — and function-pointer fields such as `delegate*<int, void> Callback;` / `delegate* unmanaged[Cdecl]<int, int> _op;`) | event, delegate (including spaced generic type tokens and pointer returns) | using, using alias, extern alias | yes |
...
SQL graph extraction also emits `reference` edges for named source/target forms such as `FROM`, `JOIN`, `INSERT INTO`, `UPDATE`, `TRUNCATE TABLE`, `DELETE FROM`, `DELETE ... USING`, `MERGE [INTO]`, and `MERGE ... USING`. T-SQL `#temp` / `##temp` targets are recognized on write-side forms such as `INSERT INTO #temp`, `INSERT TOP (...) INTO #temp`, `UPDATE #temp`, `DELETE FROM #temp`, `TRUNCATE TABLE #temp`, `SELECT ... INTO #temp`, multiline `SELECT ... INTO #temp`, `CREATE TABLE #temp`, and `MERGE [INTO] #temp ...`; later statement-order reads of temp objects established by those earlier statements, including later same-line `;`-separated statements and later semicolon-less statement boundaries such as top-level `SET`, `DECLARE`, `IF`, `WHILE`, `DO`, and `BEGIN`, also stay on the `reference` path. SQL-specific source/target modifiers such as `SELECT TOP (...)`, `INSERT TOP (...) INTO`, `UPDATE TOP (...)`, `DELETE TOP (...) FROM`, `MERGE TOP (...) [INTO]`, `UPDATE ONLY ...`, `FROM ONLY ...`, and `FROM/JOIN LATERAL ...` are skipped before resolving the real object name, `DELETE ... USING` restores PostgreSQL join-delete sources without reopening the broader DDL `USING` false positives, and `TRUNCATE TABLE ONLY public.users` plus comma-separated target lists such as `TRUNCATE TABLE audit_log, archived_log` resolve to the real targets instead of phantom `ONLY` / missing later names. Those `TRUNCATE` / `DELETE ... USING` paths now follow the same canonicalization contract as the older SQL source/target readers: qualified and quoted-qualified object paths still emit only the terminal object name (`public.users` -> `users`, `[archive].[archived_log]` -> `archived_log`), while bare PostgreSQL identifiers may also contain `$`, so unquoted names such as `my$table` stay whole instead of truncating to `my`, and existing Unicode bare identifiers such as `ユーザー` remain supported on source/target/proc-call paths. Bare `USING` is intentionally not treated as a generic source introducer because other SQL dialects reuse it for non-source clauses such as `CREATE INDEX ... USING btree (...)` and `ALTER TABLE ... USING lower(...)`; however, real expression calls inside those clauses (for example `USING lower(name)`) still remain on the `call` path. SQL Server target hints between `MERGE [INTO] <target>` and `USING <source>` are also allowed, so same-line and multiline forms such as `MERGE audit_log WITH (INDEX(ix), HOLDLOCK) AS t USING staging_log ...`, `MERGE INTO audit_log WITH (INDEX(ix), HOLDLOCK) AS t USING staging_log ...`, `MERGE INTO #audit_log` followed by a next-line `WITH (...) AS t`, and unfinished target/temp/source/hint prefixes split by a line-end `-- comment` such as `INSERT INTO -- comment`, `UPDATE -- comment`, `DELETE FROM -- comment`, `TRUNCATE TABLE -- comment`, `CREATE TABLE -- comment`, `SELECT ... INTO -- comment`, `DELETE ... USING src, -- comment`, or `MERGE ... WITH (INDEX(...), -- comment` still keep the later continuation on the same SQL statement path. `MERGE ... WHEN MATCHED THEN UPDATE SET ...` no longer emits the action-clause keyword `SET` as a table target, while quoted forms such as `UPDATE [SET] SET ...` remain valid. `EXEC` / `CALL` sites plus table-valued-function uses like `FROM dbo.fn_x(...)`, `FROM [dbo].[fn_x](...)`, ``FROM `fn_x`(...)``, and `FROM "fn_x"(...)` stay on the `call` path, while ANSI double-quoted source identifiers such as `"users"` / `"schema"."orders"` stay on the `reference` path. Bare `EXEC` / `EXECUTE` / `CALL` invocations whose target looks like `#temp` / `##temp` (MySQL / MariaDB line-comment shape) only emit a `call` edge when the same file previously establishes that `#name` via `CREATE TABLE #x`, `SELECT ... INTO #x`, a mutation target of `#x` (`INSERT INTO #x`, `UPDATE #x`, `DELETE FROM #x`, `TRUNCATE TABLE #x`), or `CREATE PROCEDURE|PROC|FUNCTION #x` (optionally `OR REPLACE|ALTER` / `TEMP|TEMPORARY` / `IF NOT EXISTS`), mirroring the existing `#temp` establishment gate used by SQL `FROM` / `JOIN` / `USING`. Without that T-SQL evidence bare `CALL #commented_out;` / `EXEC #tempProc;` / `EXECUTE #tempExecute;` stay MySQL / MariaDB comments and produce no graph edge, while quoted forms such as `EXEC [#tempProc];` and `` EXEC `#backtickProc`; `` are unambiguous identifiers and bypass the gate. Double-quoted SQL text only participates in object extraction when the introducer keyword sits outside the quoted region, so dynamic SQL assignments such as `SET @sql = "SELECT * FROM users";` do not leak phantom `reference users` rows or falsely establish later `#temp` reads. Multiline single-quoted string bodies, multiline `/* ... */` comments, and PostgreSQL dollar-quoted bodies (`$$...$$` / `$tag$...$tag$`) are blanked before source/target scanning as well, so non-code SQL text cannot leak authoritative `reference` rows or phantom temp establishment; same-line `$$...$$; SELECT ...; $$...$$` shapes close at the first real delimiter so later same-line statements are still visible. MySQL / MariaDB backslash-escaped single quotes inside `'...'` strings remain opaque to the source scanner, `# comment` tails are still stripped before graph extraction, unfinished multiline `DELETE ... USING` / `MERGE ... USING` statements keep their later `USING` source continuation even when the previous line ended with `-- comment`, and standalone procedural `SELECT ... INTO variable` does not become an object reference.
...
src/CodeIndex/
  ...
  Indexer/
    ...
    References/
      ...
      Support/
        StructuralLineMasker.cs — 参照抽出で使う文字列・コメント・構造マスク用の共有ヘルパー
        TypedLanguageReferenceExtractor.cs — 型付き言語の colon / arrow / heritage / generic type-position reference 用の共有スキャナヘルパー
        TrailingLambdaReferenceExtractor.cs — Kotlin、Scala、Swift 向け trailing lambda / block call 共有ヘルパー
        JvmMethodReferenceExtractor.cs — Java と Kotlin 向けの JVM method-reference 共有ヘルパー
        SqlNameResolver.cs    — SQL 識別子の正規化とスコープ付き名前解決ヘルパー
      Languages/
        BatchReferenceExtractor.cs — Batch 固有の `goto` / `call` label target 参照抽出
        CSharpReferenceExtractor.cs — C# 固有の constructor-chain 書き換え、type-position reference、XML-doc cref reference、qualified enum-member reference 参照抽出
...
|---|---|---|---|---|---|---|---|---|:---:|
| Python | def, async def, def NAME[T](...), async def NAME[T](...) | class | -- | -- | -- | @property | -- | from/import, type NAME = ... | yes |
| JavaScript | function, トップレベル generator / async generator（`function*`、`async function*`）, アロー, メソッド（同一行の keyword / modifier 名、default 引数、computed、`#private`、generator、`async *`、`handleClick = () => {}` のような class-field アロープロパティを含む）, オブジェクトリテラルの method / accessor / generator ショートハンド（`{ foo() {}, get foo() {}, set foo(v) {}, *bar() {}, async baz() {} }`、`module.exports = { ... }` も含む）, qualified assignments（`Vehicle.prototype.start = function () {}` / `Foo.Bar = class {}` / `Vehicle.VERSION = "1.0"`）と named function expression（`var add = function add() {}`）、`module.exports.foo = function () {}` / `module.exports.bar = () => {}` のような直接的な CommonJS named export binding、PascalCase HOC ラップ / 呼び出し結果 / タグ付きテンプレート束縛 — RHS は既知の HOC プレフィックス（`React.memo(` / `React.forwardRef(` / `React.lazy(` のみ。素の `React.` は受け付けないため、`React.createContext(...)` / `React.useCallback(...)` / `React.useMemo(...)` / `React.createRef(...)` のような非 HOC の React API 呼び出しは phantom `function` シンボルにならない。`styled.` / `styled(` / `styled\``、素の `connect(` / `memo(` / `forwardRef(` / `lazy(` / `observer(` / `with<PascalCase>(`）に限定し、`const Config = loadConfig();` / `const ENV = ...;` / `const Widget = class extends ...` が phantom `function` にならないようにする（`= class` は class expression の合成パスが担当）。加えて post-match gate（`ShouldSkipJavaScriptTypeScriptStyledFactoryCandidate`）が、元のソース文（match 終端から次の `;` または次の文まで）に実コード上のバッククォートを含まない `styled.xxx;` / `styled(x);` のような factory-capture / 素の呼び出し形を追加で拒否する。スキャナはコメント / 文字列対応で、`//` 行コメント・物理行をまたぐ `/* ... */` ブロックコメント・`'...'` / `"..."` 文字列リテラルをスキップするため、コメント内や文字列内のバッククォート・`;` は判定に寄与しない。したがって同じ物理行の別の文に無関係なタグ付きテンプレートがあってもゲートが解除されず、`styled.` / `styled(` プレフィックスはタグ付きテンプレート形（`styled.div\`...\`` / `styled(Component)\`...\``）だけが生き残る。スキャナは指定行数までの lookahead で物理行をまたいで走査するため、Prettier 整形のように `styled.div` / `styled(Component)` の次行にバッククォートを置く多行タグ付きテンプレート束縛も生き残る（継続行の最初の空白・コメント以外の文字は、タグ付きテンプレートの継続として妥当な開始文字 — バッククォート / `.` — でなければならない。`foo(\`x\`)` のような識別子始まりの式文、top-level `await foo(\`x\`)`、`const Y = 5;` のような宣言文、`<Foo>{...}` のような JSX 要素 / TS キャスト始まりはすべて暗黙 ASI による文終端として扱い走査を打ち切り、`const X = styled.div\nfoo(\`x\`)` / `const X = styled.div\nawait foo(\`x\`)` / `const X = styled.div\nconst Y = 5;` / `const X = styled.div\n<Foo>{...}` はいずれも引き続き除外される（`<` は styled-components のジェネリクスが常に同じ式内で backtick 前に書かれ、新しい行の先頭には現れないため意図的に許可しない）。さらに走査中は paren / bracket / angle / brace の depth を追跡し、`.attrs({ ... })` のようなネスト式内のバッククォートは tag head として採用されないようにする。加えて match 終端と最初のバッククォートの間に depth 0 の演算子文字（`+`、`-`、`*`、`%`、`?`、`!`、`&`、`\|`、`^`、`=`、`,`、`:`）が現れた場合は tag-head 継続チェーンが切れたとして即除外するため、`const X = styled.div + \`not a tag\`` / `const Y = styled(C) + \`also not\`` は phantom `function` にならない。regex 側で既に開き括弧（`styled(` / `memo(` / `connect(` 等）を消費している場合は scan 側 depth を -1 から始めて、対応する閉じ `)` で 0 に戻すことで、Prettier 整形の多行 `styled(Component)\n\`...\`` 形を引き続き受け付けられるようにする。さらに depth 0 の tag head バッククォートを見つけたら、テンプレート本体（`${...}` 補間を含む）を `\\` エスケープ処理付きで raw 行をまたいで末尾まで歩き、`tagHeadConsumed` フラグを立てて走査を継続する。closing backtick の **後** に depth 0 の演算子文字が現れた場合は依然として除外し（`const NotStyledPlusTheme = styled.div\`color: red\` + theme;` は phantom `function` にならない）、closing backtick の後に depth 0 の `;` が来た場合は styled 束縛として綺麗に受理する。lookahead window は物理 8 行から 32 行に拡張したため、Prettier 整形で `.attrs((props) => ({ ... }))` の引数オブジェクトがバッククォートまで 10 行以上に広がる束縛もテール側のタグ付きテンプレートまで解決できる。`depth > 0` の間は継続行の先頭文字チェックをスキップするため、`.attrs((props) => ({ ... }))` のような複数行ネスト式の 2 行目以降（例: `field1: props.value1,`）は新しい文ではなく式継続として認識される。さらに arrow 関数トークン `=>` を 1 単位として認識し、`>` が depth を誤って減らすことで arrow body 内（例: `(props) => prop1 ? a : b`）の depth 0 演算子文字が誤除外を引き起こさないようにする）; 宣言行のみの `BodyStyle.None`; 小文字始まり束縛も除外 | class, export default class, 同一行 sibling / statement-prefixed public class, クラス式, 複数行 / parenthesized / CommonJS クラス export | -- | -- | -- | `export * from` / `export * as ns from` / `export { foo as bar } from` のような ESM re-export（comment 付き / 複数行を含む）と、`module.exports = { foo: inner }` / `module.exports = { foo, bar }` のような exported object-literal の alias / shorthand property を property 的な export surface symbol として抽出 | -- | import...from | yes |
| TypeScript | function, トップレベル generator / async generator（`function*`、`async function*`）, アロー, メソッド（generic / 同一行 object-return / conditional / function-return、default 引数、computed、`#private`、generator、`async *`、`handleClick = (): void => {}` のような戻り値型付き class-field アロープロパティを含む）, オブジェクトリテラルの method / accessor / generator ショートハンド（`{ foo(): T {}, get foo(): T {}, set foo(v: T) {}, *bar(): Generator<T> {} }`）, `module.exports.foo = function () {}` / `module.exports.bar = () => {}` のような直接的な CommonJS named export binding、PascalCase HOC ラップ / 呼び出し結果 / タグ付きテンプレート束縛（オプションの `: <型>` 注釈つき） — JavaScript と同じ狭い HOC プレフィックス allowlist（`React.memo(` / `React.forwardRef(` / `React.lazy(` に厳密で、素の `React.` は受け付けないので、`const Theme: React.Context<T> = React.createContext(...)` のような非 HOC の React API 呼び出しは phantom `function` シンボルにならない）。JavaScript 行と異なり TypeScript 行は HOC 呼び出し自身に付く `<型引数>` も許容し（`React.forwardRef<HTMLDivElement, Props>(...)`、`React.memo<Props>(...)`、`connect<State, Dispatch>(...)`、`with<Pascal><T>(...)`）、3 段までの山括弧ネスト（`React.memo<Record<string, Map<string, Props>>>(...)`）と generic 引数内の丸括弧付きセグメント（`React.memo<(props: Props) => JSX.Element>(...)`）を共有定数 `TypeScriptOptionalHocTypeArgsPattern` で受理する（各 alternation 分岐は互いに素な先頭文字クラスで始まるため catastrophic backtracking は発生しない）。JavaScript 行はこのトークンを意図的に持たない。JS に generic 構文は無く、`const Result = memo < Props > (Component);` は比較・呼び出しの連鎖式であって phantom な HOC 束縛を生やしてはいけないためである。arrow 行も同じオプション注釈を受け付けるため、注釈自体に `=>` を含む型付き arrow 束縛（`const Callback: (x: number) => number = (x) => x + 1`）は arrow 行（`BodyStyle.Brace`）で捕捉され HOC 行に上書きされない。型付き HOC 束縛（`const Connected: React.ComponentType<Props> = connect(...)(Component)`、`const Styled: StyledComponent<'div', Theme> = styled.div\`...\``）は HOC 行で捕捉する。HOC 行は宣言のみの `BodyStyle.None`; 小文字始まり束縛も除外 | class, export default class, 匿名 default `abstract class`, `export = class`, 同一行 sibling / statement-prefixed public class, 複数行 / parenthesized クラス式, type | -- | interface | enum, const enum | `export * from` / `export * as ns from` / `export { foo as bar } from` / `export type { User } from` のような ESM/TS re-export（comment 付き / 複数行を含む）と、`module.exports = { foo: inner }` / `module.exports = { foo, bar }` のような exported object-literal の alias / shorthand property を property 的な export surface symbol として抽出 | -- | import...from | yes |
| C# | メソッド, コンストラクタ, explicit-interface 実装（`IMap<string, int>.GetCount` のような多引数 generic 修飾子、`IFoo<string?>.NullableArg` のような nullable 型引数修飾子、`IFoo<int[]>.ArrayArg` のような配列型引数修飾子にも対応）, インデクサ（`ref` / `ref readonly` 戻り値、`int*` / `void**` / `delegate*<int, int>` / `int*[]` のようなポインタ / 関数ポインタ戻り値、`(int, int)[]` / `(int, int)?` / `(int, int)[][]` / `(int, int)[,]` のような末尾サフィックス付き tuple 戻り値にも対応。qualified call expression の直前にある named-argument label だけを除外し、`global::` / alias-qualified な戻り値型と、スペースを含む generic 型トークンを許可。属性ストリッパは `[Obsolete, Conditional("DEBUG")]` や `[Fact, Trait("cat","io")]` のような複数セクション属性もブランク化して、2 つ目以降の属性名が phantom `function` シンボルとして漏れないようにする。さらに `from` / `where` / `select` / `orderby` / `group` / `join` / `let` / `into` / `on` / `equals` / `ascending` / `descending` / `by` のような LINQ 式 contextual keyword は戻り値型位置から除外し、`where Validator.Check(x)` / `select Mapper.Convert(x)` / `orderby Math.Abs(x)` のような continuation 行が qualified member 名を phantom `function` シンボルとして出さないようにする。加えて修飾子順序は自由で、visibility を修飾子列の任意位置に置いてよい。型宣言（`abstract public class`、`sealed public class`、`readonly public struct`、`ref public struct`、`partial public interface`、`abstract public record class`）、`const` フィールド（`new public const int X = 1;`、`public new const int X = 1;`）、`static readonly` フィールド（`readonly public static int E = 6;`）、メソッド（`static public int F() => 0;`）、プロパティ（`static public int P { get; set; }`）、インデクサ（`static public int this[int i] => 0;`）、イベント、デリゲート、演算子 / 変換演算子オーバーロード、コンストラクタ（`unsafe public S(int* p) {}`、`extern public S(int x);`）、静的コンストラクタ（`unsafe static S()`）のすべてが対象で、先頭修飾子（`static` / `public` / `private` / `protected` / `internal` / `unsafe` / `extern`）が単独行に置かれ `Name(...)` ヘッダが次行に来る折り返しコンストラクタ（`[System.Obsolete]\nstatic\nD() { ... }` のような属性付き形を含む）は wrapped-modifier リカバリで復元され、識別子行でシンボルを発行しつつ `signature` に修飾子チェーンを残す。`unsafe` / `extern` もプロパティ / インデクサ / イベント / コンストラクタ / 静的コンストラクタ行の自由順序な修飾子として受け付ける。`static` / `readonly` も `new` と任意順に並べられる（例: `readonly new static`、`new readonly static`）。C# の継承修飾子 `virtual` / `override` / `abstract` / `sealed` / `new` も event 宣言行の自由順序修飾子として受け付け（`abstract public event E;`、`sealed public override event E;`）、file スコープ型修飾子 `file` とネスト隠蔽の `new` は `interface` / `delegate` 行でも受け付ける（`file interface I {}`、`file delegate int D(int x);`））, `operator +` / `operator checked +` / `operator checked -` 形式で保持する演算子（C# 11 のユーザー定義 `operator checked` を単項・二項とも含む）, `implicit operator decimal` / `explicit operator Money` / `explicit operator checked int` 形式で保持する変換演算子（`unsafe` / `extern` 付きと function-pointer target type を含む）, `Item` に正規化するインデクサ, const, static readonly, enum メンバー, #region, ファイナライザ | class, record（折り返されたヘッダでは base list と `where` 句を `symbols.signature` に保持、C# 12 primary constructor パラメータリストも含む） | struct, record struct, ref struct（折り返されたヘッダでは base list と `where` 句を signature に保持） | interface（折り返されたヘッダでは base list と `where` 句を signature に保持） | enum（折り返されたヘッダでは `: underlyingType` を signature に保持） | property, partial property, 式本体, `ref` / `ref readonly` property, ポインタ property, tuple-array / nullable-tuple property, 明示的インターフェースプロパティ実装（ブレース本体の `int IThing.Value { get; set; }` と式本体の `string IThing.Name => "x";` の両形、`IBucket<int>.Items` のような generic interface 修飾子、`IMap<string, int>.PairCount` のような多引数 generic 修飾子も含む）, record primary component, 通常フィールド（instance / readonly / volatile / 通常 static、初期化子の有無を問わず、`;` 前で折り返す multi-line 宣言 — `= new(\n    () => 42);` のような括弧付き / コンストラクタ呼び出し初期化子や `= new() { ... };` / `= new Dictionary<...> { ... };` のようなオブジェクト / コレクション初期化子を含む — `int _x, _y;` のような declarator list — declarator ごとに 1 シンボル — および `delegate*<int, void> Callback;` / `delegate* unmanaged[Cdecl]<int, int> _op;` のような function-pointer フィールド） | event, delegate（generic 型引数内スペースを含む形と、ポインタ戻り値の delegate 宣言も対応） | using, using alias, extern alias | yes |
| C# | メソッド, コンストラクタ, explicit-interface 実装（`IMap<string, int>.GetCount` のような多引数 generic 修飾子、`IFoo<string?>.NullableArg` のような nullable 型引数修飾子、`IFoo<int[]>.ArrayArg` のような配列型引数修飾子にも対応）, インデクサ（`ref` / `ref readonly` 戻り値、`int*` / `void**` / `delegate*<int, int>` / `int*[]` のようなポインタ / 関数ポインタ戻り値、`(int, int)[]` / `(int, int)?` / `(int, int)[][]` / `(int, int)[,]` のような末尾サフィックス付き tuple 戻り値にも対応。qualified call expression の直前にある named-argument label だけを除外し、`global::` / alias-qualified な戻り値型と、スペースを含む generic 型トークンを許可。属性ストリッパは `[Obsolete, Conditional("DEBUG")]` や `[Fact, Trait("cat","io")]` のような複数セクション属性もブランク化して、2 つ目以降の属性名が phantom `function` シンボルとして漏れないようにする。さらに `from` / `where` / `select` / `orderby` / `group` / `join` / `let` / `into` / `on` / `equals` / `ascending` / `descending` / `by` のような LINQ 式 contextual keyword は戻り値型位置から除外し、`where Validator.Check(x)` / `select Mapper.Convert(x)` / `orderby Math.Abs(x)` のような continuation 行が qualified member 名を phantom `function` シンボルとして出さないようにする。加えて修飾子順序は自由で、visibility を修飾子列の任意位置に置いてよい。型宣言（`abstract public class`、`sealed public class`、`readonly public struct`、`ref public struct`、`partial public interface`、`abstract public record class`）、`const` フィールド（`new public const int X = 1;`、`public new const int X = 1;`）、`static readonly` フィールド（`readonly public static int E = 6;`）、メソッド（`static public int F() => 0;`）、プロパティ（`static public int P { get; set; }`）、インデクサ（`static public int this[int i] => 0;`）、イベント、デリゲート、演算子 / 変換演算子オーバーロード、コンストラクタ（`unsafe public S(int* p) {}`、`extern public S(int x);`。コンストラクタ regex の開き括弧直後に位置検査の否定先読みを入れ、「対応する `)` のあとに識別子 + `{` / `(` / `=>`（間に `?` / `[]` / `[,]` / 空白混じりの tuple サフィックスを許容。例: `) []` / `) ?` / `)  ?`。サフィックスのバリエーションは共有定数 `CSharpTupleSuffixPattern` に切り出し、`CSharpTypePattern` と ctor lookahead の双方から参照することで両者の整合を保つ）が続く行」——modifier 付き property や式本体メソッドそのものの形——を ctor 候補から弾く。これにより、上流の method / property regex が失敗した行——たとえば `public required (int, int) R1 { get; init; }` や `public required (int, int) [] R4 { get; init; }` が以前 phantom `function required` を出していたケース——でも、`partial` / `required` / `readonly` / `async` / `sealed` / `virtual` / `override` / `abstract` / `new` / `file` / `static` のような modifier キーワードが ctor 名として拾われない。複数行 ctor シグネチャ、`;` で終わる `extern` ctor、式本体 ctor、`: base(...)` / `: this(...)` 初期化子、tuple パラメータ ctor はすべて影響を受けず引き続き抽出される）、静的コンストラクタ（`unsafe static S()`）のすべてが対象で、`unsafe` / `extern` もプロパティ / インデクサ / イベント / コンストラクタ / 静的コンストラクタ行の自由順序な修飾子として受け付ける。`static` / `readonly` も `new` と任意順に並べられる（例: `readonly new static`、`new readonly static`）。C# の継承修飾子 `virtual` / `override` / `abstract` / `sealed` / `new` も event 宣言行の自由順序修飾子として受け付け（`abstract public event E;`、`sealed public override event E;`）、file スコープ型修飾子 `file` とネスト隠蔽の `new` は `interface` / `delegate` 行でも受け付ける（`file interface I {}`、`file delegate int D(int x);`））, `operator +` / `operator checked +` / `operator checked -` 形式で保持する演算子（C# 11 のユーザー定義 `operator checked` を単項・二項とも含む）, `implicit operator decimal` / `explicit operator Money` / `explicit operator checked int` 形式で保持する変換演算子（`unsafe` / `extern` 付きと function-pointer target type を含む）, `Item` に正規化するインデクサ, const, static readonly, enum メンバー, #region, ファイナライザ | class, record（折り返されたヘッダでは base list と `where` 句を `symbols.signature` に保持、C# 12 primary constructor パラメータリストも含む） | struct, record struct, ref struct（折り返されたヘッダでは base list と `where` 句を signature に保持） | interface（折り返されたヘッダでは base list と `where` 句を signature に保持） | enum（折り返されたヘッダでは `: underlyingType` を signature に保持） | property, partial property, 式本体, `ref` / `ref readonly` property, ポインタ property, tuple-array / nullable-tuple property, 明示的インターフェースプロパティ実装（ブレース本体の `int IThing.Value { get; set; }` と式本体の `string IThing.Name => "x";` の両形、`IBucket<int>.Items` のような generic interface 修飾子、`IMap<string, int>.PairCount` のような多引数 generic 修飾子も含む）, record primary component, 通常フィールド（instance / readonly / volatile / 通常 static、初期化子の有無を問わず、`;` 前で折り返す multi-line 宣言 — `= new(\n    () => 42);` のような括弧付き / コンストラクタ呼び出し初期化子や `= new() { ... };` / `= new Dictionary<...> { ... };` のようなオブジェクト / コレクション初期化子を含む — `int _x, _y;` のような declarator list — declarator ごとに 1 シンボル — および `delegate*<int, void> Callback;` / `delegate* unmanaged[Cdecl]<int, int> _op;` のような function-pointer フィールド） | event, delegate（generic 型引数内スペースを含む形と、ポインタ戻り値の delegate 宣言も対応） | using, using alias, extern alias | yes |
...
C# の `Graph = yes` は callable/reference extraction と event subscription を指します。`new Dictionary<string, List<int>>()` のような nested generic コンストラクタ呼び出しや `Helper.DoWork<List<int>>()` のような C# generic method call も depth-aware fallback を通して同じ graph 経路に乗るため、外側の callee/constructor target が `references` / `callers` / `callees` / `impact` に残ります。Shell is intentionally excluded because its command-style invocations (`foo arg1 arg2`) cannot be detected by the parenthesized-call regex. PowerShell uses a dedicated command-start pass so hyphenated cmdlet/function invocations such as `Get-ChildItem`, `Write-Host`, and `$items | ForEach-Object { ... }` are emitted as call edges even without parentheses. C# の型位置 edge には `is` / `is not` / `as`、switch 文の型パターン（`case Point:` / `case Point when value != null:` / `case ILogger logger:` / `case Base _:` / `case Point { X: 0, Y: 0 }:` / `case Point(var x, var y):` / `case Point or null:` / `case not Point:` / `case Probe.Point or null:` / `case not Probe.Point:` / `case global::Probe.Point or null:` / `case N2.Color.Red r:` / `case Outer.Red or Outer.Blue:` / `case Color.Red or Point:` など）、さらに switch 式 arm head（`Point =>` / `Point or Shape =>` / `Wrapper<Point, Shape> =>` / `Point { X: 0, Y: 0 } =>` など）も含まれます。さらに、この複数行 carry は単独継続行の `not` もまたげるため、`value is` / `not` / `Point` や `case` / `not` / `Point:` のような形でも、本物の `Point` 依存を head 行まで保ったまま抽出します。logical pattern に複数の genuine type head がある場合は、それぞれの genuine head ごとに `type_reference` 行が出るため、`case Point or Shape:` の `Shape`、`Point or Shape =>` の `Shape`、`Wrapper<Point, Shape> =>` の `Wrapper` / `Point` / `Shape`、`case Outer.Red or Outer.Blue:` の `Blue` のような後続 head も `references` / `inspect` / `impact` から見え続けます。`case List<int> [_, ..]:` のような list pattern でも先頭型は `type_reference` として残ります。さらに `case Foo @or:` / `case Foo @when:` / `case Foo @and:` のような verbatim pattern variable でも enclosing `Foo` の型依存は維持され、`value is @not` / `case @default:` / `typeof(@default)` のような escaped type 名も bare token と誤認されず、正規化済み `type_reference` 行 (`not` / `default`) を出します。加えて cross-file の `using static` import も pattern 位置の read path で考慮されるようになり、`using static Probe.Color; value is Red or Blue;`、`case Red:`、`case Red or Blue:` のような imported constant/member pattern は `Red` / `Blue` の phantom `type_reference` 行を漏らさず、`case Red or Point:` の `Point` のような genuine type head は引き続き残ります。この read path 抑止は、active な C# スコープで見える同名の本物の型候補がある場合にはそれを優先し、表示幅で切り詰めた preview ではなく raw line text を見た上で、別ファイルにある project-wide `global using static ...;` / `global using Namespace;` も active import 集合へ加え、`using Alias = ...;` は unqualified namespace import rescue から除外し、active な `using Red = Some.Type;` 型 alias も suppress 回避用の本物の同名型候補として扱い、same-file の extractor 抑止も containing type scope と same-file の `using Namespace;` import まで同じ規則を反映するため、`Red` が実在の in-scope 型や enclosing type 内の local type である `value is Red` / `typeof(Red)` は、同じ短名の enum member が `using static` で入っていても消されません。この suppress 経路の count-only read path も preview-ready な `ReferenceResult` を構築せず raw row を streaming で数えます。一方で `x is not null`、`x is default`、`case default:`、`value is Color.Red or Color.Blue`、`case Color.Red:` / `case Color.Red or Namespace.Color.Blue:` / `case ErrorCodes.NotFound or ErrorCodes.Forbidden:`、`Color.Red =>` のような定数/メンバー パターンは非型の参照経路のままで、`type_reference` 行を出してはなりません。`@class` / `@return` / `@if` のような verbatim identifier も C# の宣言名・call/instantiate 経路で受理され、保存される symbol/reference 名は qualified な namespace/import/container path（`Outer.@class` → `Outer.class`）や conversion operator の対象型（`implicit operator List<@class>` → `implicit operator List<class>`）を含めて先頭 `@` を外した意味上の識別子へ正規化されるため、lookup と graph matching は綴りの表記差分に依存しません。`Nested.A` や `Outer.First.None` のような qualified enum-member access も索引され、参照行は symbol extractor が復元できる最も内側の owner（クラス全体ではなく property / method など）を引き継ぎます。一方で `unused` は別の制約がまだ残っており、C# enum member を含む scope はその member が `unused` から除外されるため degraded として扱われ、enum declaration も偽陽性になりうるものとして残ります。
...
SQL の graph 抽出は `FROM` / `JOIN` / `INSERT INTO` / `UPDATE` / `TRUNCATE TABLE` / `DELETE FROM` / `DELETE ... USING` / `MERGE [INTO]` / `MERGE ... USING` のような名前付き source/target 形も `reference` edge として出力します。T-SQL の `#temp` / `##temp` は `INSERT INTO #temp`、`INSERT TOP (...) INTO #temp`、`UPDATE #temp`、`DELETE FROM #temp`、`SELECT ... INTO #temp`、複数行 `SELECT ... INTO #temp`、`CREATE TABLE #temp`、`MERGE [INTO] #temp ...` のような target 側で認識し、その temp object が先行 statement によって確立された後続 read、同一行の `;` 区切り statement と top-level `SET` / `DECLARE` / `IF` / `WHILE` / `DO` / `BEGIN` を含む semicolon-less な後続 statement 境界でも statement-order の後続 read として `reference` 経路に残します。`SELECT TOP (...)`、`INSERT TOP (...) INTO`、`UPDATE TOP (...)`、`DELETE TOP (...) FROM`、`MERGE TOP (...) [INTO]`、`UPDATE ONLY ...`、`FROM ONLY ...`、`FROM/JOIN LATERAL ...` のような SQL 専用 modifier は実 object 名を解決する前に読み飛ばし、`DELETE ... USING` では PostgreSQL の join-delete source を DDL `USING` の偽陽性を戻さずに復元し、`TRUNCATE TABLE ONLY public.users` や `TRUNCATE TABLE audit_log, archived_log` のような target も phantom `ONLY` や後続 target 欠落なしで復元します。これらの `TRUNCATE` / `DELETE ... USING` 経路も既存の SQL source/target 読み取りと同じ canonicalization 契約に従い、qualified / quoted-qualified な object path でも terminal object 名だけ（`public.users` -> `users`、`[archive].[archived_log]` -> `archived_log`）を emit します。さらに PostgreSQL の bare identifier は `$` を含められるため、`my$table` のような unquoted 名も `my` に切らず識別子全体を保持し、`ユーザー` のような既存の Unicode bare identifier も source/target/proc-call 経路で引き続き有効です。bare な `USING` は `CREATE INDEX ... USING btree (...)` や `ALTER TABLE ... USING lower(...)` のような非 source 句にも再利用されるため、汎用 source 導入子としては扱いません。ただし、それらの句に含まれる本物の式呼び出し（例: `USING lower(name)`）は引き続き `call` 経路に残します。さらに SQL Server で `MERGE [INTO] <target>` と `USING <source>` の間に入る target hint も許容するため、`MERGE audit_log WITH (INDEX(ix), HOLDLOCK) AS t USING staging_log ...`、`MERGE INTO audit_log WITH (INDEX(ix), HOLDLOCK) AS t USING staging_log ...`、`MERGE INTO #audit_log` の次行に `WITH (...) AS t` が来る複数行形に加え、`INSERT INTO -- comment`、`UPDATE -- comment`、`DELETE FROM -- comment`、`TRUNCATE TABLE -- comment`、`CREATE TABLE -- comment`、`SELECT ... INTO -- comment`、`DELETE ... USING src, -- comment`、`MERGE ... WITH (INDEX(...), -- comment` のように未完了 target/temp/source/hint prefix の途中へ行末コメントが入る形でも後続の継続を保持します。`MERGE ... WHEN MATCHED THEN UPDATE SET ...` の action clause からは `SET` を target reference として出さず、`UPDATE [SET] SET ...` のような quoted target は正当な target として維持します。`EXEC` / `CALL` と `FROM dbo.fn_x(...)` / `FROM [dbo].[fn_x](...)` / ``FROM `fn_x`(...)`` / `FROM "fn_x"(...)` のような table-valued function 使用は引き続き `call` 経路に残し、ANSI の二重引用符付き source identifier（`"users"` / `"schema"."orders"` など）は `reference` 経路に残します。bare な `EXEC` / `EXECUTE` / `CALL` の呼び出し先が `#temp` / `##temp`（MySQL / MariaDB の行コメント形）に見える場合は、同一ファイル内で先に `CREATE TABLE #x` / `SELECT ... INTO #x` / `#x` を変更対象とする更新（`INSERT INTO #x` / `UPDATE #x` / `DELETE FROM #x` / `TRUNCATE TABLE #x`）/ `CREATE PROCEDURE|PROC|FUNCTION #x`（`OR REPLACE|ALTER` / `TEMP|TEMPORARY` / `IF NOT EXISTS` の有無は問わない）で該当 `#name` が確立されているときだけ `call` edge を出すようになりました。これは SQL の `FROM` / `JOIN` / `USING` が既に採用している `#temp` 確立ゲートと揃えたもので、確立のない bare な `CALL #commented_out;` / `EXEC #tempProc;` / `EXECUTE #tempExecute;` は MySQL / MariaDB の行コメントのまま graph edge を出しません。一方で `EXEC [#tempProc];` や `` EXEC `#backtickProc`; `` のような引用形は識別子として明確なので、このゲートを通さず従来どおり edge を出します。二重引用符付き SQL テキストは導入キーワード自体が引用内にある限り object 抽出の対象にしないため、`SET @sql = \"SELECT * FROM users\";` のような dynamic SQL 文字列から phantom `reference users` も、未実行の `SELECT ... INTO #temp` による phantom temp establishment も漏れません。さらに複数行の single-quoted 文字列本体、複数行 `/* ... */` コメント、PostgreSQL の dollar-quoted body（`$$...$$` / `$tag$...$tag$`）も source/target 走査前に空白化するため、非コード SQL テキストから authoritative な `reference` row や phantom temp establishment は漏れません。同一行の `$$...$$; SELECT ...; $$...$$` 形では最初の実 close delimiter で body を閉じるため、後続の same-line statement も見えるままです。MySQL / MariaDB の `'...'` 文字列内にある backslash escape 付き single quote も source scanner には不透明のまま保ち、`# comment` 末尾は graph 抽出前に除去し、未完了の複数行 `DELETE ... USING` / `MERGE ... USING` は前行が `-- comment` で終わっても後続の `USING` source 継続を保持し、単独の手続き系 `SELECT ... INTO variable` は object reference にしません。
