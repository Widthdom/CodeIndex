...
| Security defaults | On POSIX systems, `.cdidx` is created with `0700` permissions, lifecycle, metrics, and MCP audit logs are created owner-read/write from the start, metrics and audit logs rotate to bounded slots, and `status --json` reports the effective `data_dir_mode` when available. |
...
| security defaults | POSIX では `.cdidx` を `0700` 権限で作成し、lifecycle log、metrics log、MCP audit log は作成時点から owner read/write のみで作成します。metrics log と audit log は bounded slot へ rotation します。`status --json` は利用可能な場合に実効 POSIX mode を `data_dir_mode` として報告します。 |
