...
cdidx symbols Run --exact              # exact name match (no `RunAsync` / `RunImpact` expansion)
...
| `--exact` | `search`, `symbols`, `definition`, `references`, `callers`, `callees`, `inspect` | `search`: case-sensitive exact substring (no FTS5). Symbol/graph commands (and `inspect` / MCP `analyze_symbol` — propagates to every bundled sub-query): NFKC + invariant-lower fold for exact name match (so `Ä` / `ä`, `Ｒｕｎ` / `Run`, ligatures collapse). NOT full Unicode CaseFold — a few edge cases like Turkish `İ`/`i` and Greek final sigma still require exact casing (tracked in #96). Falls back to ASCII `COLLATE NOCASE` on legacy DBs that have not been reindexed since #86; `status --json` exposes `fold_ready` so AI clients can tell which path is active. When `--exact` returns 0 results on `symbols` / `definition` / `references` / `callers` / `callees`, cdidx re-runs the same query in substring mode and — if that would have matched — attaches an `exact_zero_hint` with the relaxed count plus up to 5 sample names so AI clients can tell a typo from a genuine miss (#88). |
...
0. **Triage every unmerged branch and open PR.** Before bumping the
   version, first refresh remote refs with `git fetch --all --prune` so
   that recently pushed release-relevant branches actually appear in the
   local listing. Then list **all** unmerged candidates — do not pre-filter
   by branch name, because a release-relevant fix can live on any branch:
   - `git branch -a --no-merged main` — every branch not yet merged into `main`.
   - `gh pr list --state open --limit 1000` — every open PR. The explicit
     `--limit` is required because `gh pr list` defaults to `--limit 30`
     and would otherwise silently truncate on busy repos.

   For each entry, explicitly decide: (a) merge it before the release,
   or (b) acknowledge in the release PR description why it is being
   deferred. `fix/**` and `codex/**` are the common naming conventions
   in this repo, but the gate is intentionally name-agnostic so a
   differently named branch cannot silently slip through. This closes
   the gap that caused the v1.8.1 release to ship without the
   unused-null-ordinal fix from `fix/unused-null-ordinal-58`, which was
   then re-reported as a post-release crash.
...
cdidx symbols Run --exact              # 名前の完全一致（`RunAsync` / `RunImpact` に広がらない）
...
- 候補シンボル名が決まっていて、定義・caller・callee・参照をまとめて欲しいときは `inspect` を使う。leaf コマンドと同じ precision にしたいときは `--exact` を付ける（bundle 内の全 sub-query に伝播するため、`inspect Run --exact` が `RunAsync` / `RunImpact` を巻き込むことはない）。`inspect --json` には `workspace_indexed_at`、`workspace_latest_modified`、`project_root`、`git_head`、`git_is_dirty` も含まれるため、信頼判断も同時に行える。
...
0. **未マージブランチと open PR をすべてトリアージする。** バージョンを
   上げる前に、まず `git fetch --all --prune` でリモート参照を更新する
   こと（直近に push されたリリース関連ブランチをローカル一覧に確実に
   表示させるため）。そのうえで、ブランチ名で事前フィルタせず、**すべての**
   未マージ候補を列挙すること。リリース影響のある修正は任意のブランチ名
   に乗り得るため、名前で絞り込むと見落としが生じる:
   - `git branch -a --no-merged main` — `main` に未マージのブランチ全件。
   - `gh pr list --state open --limit 1000` — open 状態の PR 全件。
     `gh pr list` は既定で `--limit 30` にトランケートされるため、
     明示的に `--limit` を指定しないと activity の多いリポジトリで
     黙って取りこぼしが発生する。

   各エントリについて、(a) リリース前にマージする、または (b) リリース
   PR の説明で明示的に見送り理由を残す、のどちらかを必ず選ぶ。このリポジトリ
   では `fix/**` と `codex/**` がよく使われる命名だが、別名のブランチが
   黙って素通りしないよう、このゲートは意図的に命名に依存しない。これは
   v1.8.1 のリリースが `fix/unused-null-ordinal-58` の修正を取り込まない
   まま公開され、リリース後に再報告（#60）となったプロセスギャップを
   塞ぐためのチェックです。
