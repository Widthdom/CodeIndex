...
- Use `references`, `callers`, and `callees` for symbol-aware call graph questions in Python, JavaScript/TypeScript, C#, Go, Rust, Java, Kotlin, Ruby, C/C++, PHP, Swift, Dart, Scala, Elixir, Lua, and VB.NET (18 languages). F# uses space-separated call syntax so graph queries are not supported; use `search` instead. Add `--exact` when you already have a precise name (e.g. resolved from `symbols --exact` / `inspect`) so `references Run` no longer pulls in `RunAsync` / `RunImpact` / etc. — same for `callers` / `callees` / `definition`. C# enum members are indexed as symbols, but enum-member access edges such as `Nested.A` are not indexed yet; when the active query scope includes those enum-member candidates, `inspect` keeps `graph_supported` aligned with any callable candidates while surfacing `graph_degraded=true` / `unsupported_symbol_kind=enum_member`, and exact `references` / `callers` / `callees` surface the same gap in both zero/count payloads and successful JSON rows.
...
- Python、JavaScript/TypeScript、C#、Go、Rust、Java、Kotlin、Ruby、C/C++、PHP、Swift、Dart、Scala、Elixir、Lua、VB.NET の call graph 系調査では `references`、`callers`、`callees` を使う（18 言語）。F# は空白区切りの呼び出し構文を使うため graph query は未対応で、代わりに `search` を使う。名前がすでに確定している場合は `--exact` を付けると `Run` が `RunAsync` / `RunImpact` に広がらず、`definition` も同様に `--exact` をサポートする。C# の enum member は symbol として索引される一方、`Nested.A` のような enum-member access edge はまだ索引していないため、active scope にその enum member 候補が含まれる場合、`inspect` は callable 候補があれば `graph_supported` を維持したまま `graph_degraded=true` / `unsupported_symbol_kind=enum_member` を返し、exact `references` / `callers` / `callees` も zero/count payload と成功した JSON row の両方で同じ制約を返す。
