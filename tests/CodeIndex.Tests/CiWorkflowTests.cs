...
public class CiWorkflowTests
{
    ...
    public void DotnetWorkflow_RunsTestsWithRunsettingsBlameRetryAndArtifacts()
    {
        ...
        AssertContainsAll(
            ...
            "Enabling crash diagnostics only on the failed-run retry so clean lanes avoid collector overhead.",
            "$retryExitCode = Invoke-TestRun -LogPath $retryLogPath -ResultFileName \"test_results_retry.trx\" -IncludeCoverage $false -IncludeCrashDiagnostics $true",
            "\"--no-build\"",
            "\"--no-restore\"",
            "$runArgs += \"--blame-crash\"",
