...
public partial class SymbolExtractorTests
{
    ...
    public void Extract_CSharp_WrappedNestedPartialTypesKeepRootToLeafFamilyKeys()
    {
        ...
        // types flip their arity-aware family key order (for example, starting with `Host`)
        // and no longer match the namespace/type-boundary-aware identity used by hotspot-family
        // grouping. Pin a wrapped nested partial-type fixture so both the container path and
        // family key stay in canonical root-to-leaf order. Closes #541.
        ...
        // type の arity-aware family key が `Host` から始まるように逆順化し、
        // hotspot-family grouping が使う namespace/type 境界を区別する identity と食い違う。
        ...
        Assert.Equal("ReviewFixtures+Host+Wrapped`1", wrapped.FamilyKey);
