...
public partial class QueryCommandRunnerTests
{
    ...
    public void GraphCommands_ReferenceCapHitMarksAbsenceQueriesIncomplete_Issue4620()
    {
        ...
        try
        {
            ...
            using (var depsDocument = ParseJsonOutput(depsStdout))
            {
                ...
                Assert.True(depsDocument.RootElement.GetProperty("query_exhausted").GetBoolean());
                Assert.Equal(0, depsDocument.RootElement.GetProperty("total_count").GetInt32());
                Assert.False(depsDocument.RootElement.GetProperty("total_count_authoritative").GetBoolean());
                Assert.Equal("reference_graph_incomplete", depsDocument.RootElement.GetProperty("total_count_non_authoritative_reason").GetString());
            }
    ...
    [Fact]
    public void RunDeps_JsonSummaryReportsPageAndCandidateAuthority_Issue5346()
    {
        using var project = TestProjectHelper.CreateTempProjectScope("cdidx_deps_summary_authority");
        var dbPath = TestProjectHelper.CreateProjectDb(project.Root);
        TestProjectHelper.WriteTextFile(project.Root, "src/Target.cs", "public class SummaryTarget { }");
        foreach (var path in new[] { "src/First.cs", "src/Second.cs", "src/Third.cs", "tests/CallerTests.cs" })
            TestProjectHelper.WriteTextFile(project.Root, path,
                $"public class {Path.GetFileNameWithoutExtension(path)} {{ public SummaryTarget Run() => new SummaryTarget(); }}");
        TestProjectHelper.WriteTextFile(project.Root, "src/Generated.g.cs",
            "// <auto-generated />\npublic class Generated { public SummaryTarget Run() => new SummaryTarget(); }");
        ...
        {
            var (exitCode, stdout, stderr) = CaptureConsole(() => QueryCommandRunner.RunDeps(
                ["--db", dbPath, "--json", "--summary-only", "--lang", "csharp", "--exclude-tests", "--limit", limit.ToString(System.Globalization.CultureInfo.InvariantCulture)],
                _jsonOptions));
            Assert.Equal(CommandExitCodes.Success, exitCode);
            Assert.Empty(stderr);
            using var document = ParseJsonOutput(stdout);
            var json = document.RootElement;
            Assert.Equal(Math.Min(limit, 3), json.GetProperty("count").GetInt32());
            Assert.Equal(json.GetProperty("count").GetInt32(), json.GetProperty("returned_count").GetInt32());
            Assert.Equal("returned", json.GetProperty("count_kind").GetString());
            Assert.Equal("dependency_edges", json.GetProperty("count_unit").GetString());
            Assert.Equal(limit, json.GetProperty("page_limit").GetInt32());
            Assert.Equal(limit < 3, json.GetProperty("has_more").GetBoolean());
            Assert.Equal(limit < 3, json.GetProperty("truncated").GetBoolean());
            Assert.True(json.GetProperty("candidate_scan_complete").GetBoolean());
            Assert.Equal(limit >= 3, json.GetProperty("query_exhausted").GetBoolean());
            Assert.Equal(limit >= 3, json.GetProperty("total_count_available").GetBoolean());
            Assert.Equal(limit >= 3, json.GetProperty("total_count_authoritative").GetBoolean());
            if (limit >= 3)
                Assert.Equal(3, json.GetProperty("total_count").GetInt32());
            else
            {
                Assert.Equal(JsonValueKind.Null, json.GetProperty("total_count").ValueKind);
                Assert.Equal("page_limit", json.GetProperty("total_count_unavailable_reason").GetString());
            }
            Assert.False(json.TryGetProperty("edges", out _));
        }

        foreach (var (filter, value, expectedCount) in new[]
        {
            ("--path", "src/First.cs", 1),
            ("--path", "missing/**", 0),
            ("--symbol", "MissingSymbol", 0),
            ("--symbol-family", "Summary", 3),
        })
        {
            var (exitCode, stdout, _) = CaptureConsole(() => QueryCommandRunner.RunDeps(
                ["--db", dbPath, "--json", "--summary-only", "--lang", "csharp", "--exclude-tests", filter, value, "--limit", "3", "--max-json-bytes", "4096"],
                _jsonOptions));
            Assert.Equal(CommandExitCodes.Success, exitCode);
            Assert.True(System.Text.Encoding.UTF8.GetByteCount(stdout) <= 4096);
            using var document = ParseJsonOutput(stdout);
            var json = document.RootElement;
            Assert.Equal(expectedCount, json.GetProperty("count").GetInt32());
            Assert.Equal(expectedCount, json.GetProperty("total_count").GetInt32());
            Assert.False(json.GetProperty("has_more").GetBoolean());
            Assert.True(json.GetProperty("query_exhausted").GetBoolean());
        }
        ...
        foreach (var filters in new[]
        {
            ...
            new[] { "--resolution-state", "resolved" },
            new[] { "--reverse", "--path", "src/Target.cs" },
            new[] { "--exclude-path", "src/Third.cs" },
        })
        {
            var args = new[] { "--db", dbPath, "--json", "--lang", "csharp", "--exclude-tests", "--limit", "10" }.Concat(filters).ToArray();
            var (detailExit, detailStdout, _) = CaptureConsole(() => QueryCommandRunner.RunDeps(args, _jsonOptions));
            var (summaryExit, summaryStdout, _) = CaptureConsole(() => QueryCommandRunner.RunDeps([.. args, "--summary-only"], _jsonOptions));
            Assert.Equal(CommandExitCodes.Success, detailExit);
            Assert.Equal(detailExit, summaryExit);
            using var detail = ParseJsonOutput(detailStdout);
            using var summary = ParseJsonOutput(summaryStdout);
            Assert.Equal(detail.RootElement.GetProperty("count").GetInt32(), summary.RootElement.GetProperty("count").GetInt32());
            Assert.True(summary.RootElement.GetProperty("query_exhausted").GetBoolean());
            if (filters[0] == "--include-generated")
                Assert.Equal(4, summary.RootElement.GetProperty("count").GetInt32());
        }

        using var member = TestProjectHelper.CreateTempProjectScope("cdidx_deps_summary_member");
        var memberDb = TestProjectHelper.CreateProjectDb(member.Root);
        var (workspaceExit, workspaceStdout, _) = CaptureConsole(() => QueryCommandRunner.RunDeps(
            ["--db", dbPath, "--workspace-db", memberDb, "--json", "--summary-only", "--limit", "1", "--exclude-tests"], _jsonOptions));
        Assert.Equal(CommandExitCodes.Success, workspaceExit);
        using (var workspace = ParseJsonOutput(workspaceStdout))
        {
            Assert.Equal(1, workspace.RootElement.GetProperty("count").GetInt32());
            Assert.Equal(JsonValueKind.Null, workspace.RootElement.GetProperty("has_more").ValueKind);
            Assert.False(workspace.RootElement.GetProperty("query_exhausted").GetBoolean());
            Assert.Equal("workspace_candidate_scan_unverified", workspace.RootElement.GetProperty("total_count_unavailable_reason").GetString());
        }

        var input = JsonSerializer.Serialize(new[] { "deps", "--json", "--summary-only", "--lang", "csharp", "--exclude-tests", "--limit", "1" }) + "\n";
        var (batchExit, batchStdout, batchStderr) = CaptureConsoleWithInput(input,
            () => QueryCommandRunner.RunBatch(["--db", dbPath, "--json-summary"], _jsonOptions));
        Assert.Equal(CommandExitCodes.Success, batchExit);
        Assert.Empty(batchStderr);
        var batchLines = ParseJsonLines(batchStdout);
        try
        {
            var child = batchLines[0].RootElement.GetProperty("result");
            Assert.Equal(1, child.GetProperty("count").GetInt32());
            Assert.Equal("returned", child.GetProperty("count_kind").GetString());
            Assert.True(child.GetProperty("has_more").GetBoolean());
            Assert.False(child.GetProperty("query_exhausted").GetBoolean());
        }
        finally
        {
        ...
        var (budgetExit, budgetStdout, budgetStderr) = CaptureConsole(() => QueryCommandRunner.RunDeps(
            ["--db", dbPath, "--json", "--summary-only", "--limit", "1", "--max-json-bytes", "1"], _jsonOptions));
        Assert.Equal(CommandExitCodes.UsageError, budgetExit);
        Assert.Empty(budgetStderr);
        using var budgetDocument = ParseJsonOutput(budgetStdout);
        Assert.Equal(CommandErrorCodes.ResponseBudgetTooSmall, budgetDocument.RootElement.GetProperty("error_code").GetString());
    }
    ...
    public void RunDeps_JsonSummaryDoesNotInferExhaustionFromBoundedCSharpCandidates_Issue5346()
    {
        ...
        var dbPath = TestProjectHelper.CreateProjectDb(project.Root);
        var names = Enumerable.Range(0, DependencyNoiseProfile.GetRankingCandidateLimit(1) + 1)
            .Select(i => $"SummaryTarget{i}").ToArray();
        TestProjectHelper.WriteTextFile(project.Root, "src/Target.cs",
            string.Join("\n", names.Select(name => $"public class {name} {{ }}")));
        TestProjectHelper.WriteTextFile(project.Root, "src/Caller.cs",
            "public class Caller { public void Run() {\n"
        ...
        var (exitCode, stdout, stderr) = CaptureConsole(() => QueryCommandRunner.RunDeps(
            ["--db", dbPath, "--json", "--summary-only", "--lang", "csharp", "--limit", "2"], _jsonOptions));
        Assert.Equal(CommandExitCodes.Success, exitCode);
        Assert.Empty(stderr);
        using var document = ParseJsonOutput(stdout);
        var json = document.RootElement;
        ...
        Assert.False(json.GetProperty("candidate_scan_complete").GetBoolean());
        Assert.False(json.GetProperty("query_exhausted").GetBoolean());
        Assert.True(json.GetProperty("truncated").GetBoolean());
        Assert.Equal(JsonValueKind.Null, json.GetProperty("has_more").ValueKind);
        Assert.Equal(JsonValueKind.Null, json.GetProperty("total_count").ValueKind);
        Assert.False(json.GetProperty("total_count_authoritative").GetBoolean());
        Assert.Equal("candidate_scan_incomplete", json.GetProperty("total_count_unavailable_reason").GetString());
    }
    ...
    public void RunDeps_MissingGraphJsonModesShareZeroPayloadFixture_Issues4112And4619()
    {
        ...
        try
        {
            ...
            Assert.False(summaryJson.GetProperty("query_exhausted").GetBoolean());
            Assert.False(summaryJson.GetProperty("total_count_authoritative").GetBoolean());
            Assert.Equal(JsonValueKind.Null, summaryJson.GetProperty("has_more").ValueKind);
    ...
    public void GraphCommands_BodyOptionAddsCappedBodyExcerpt_Issue1594()
    {
        ...
        try
        {
            ...
            var callsiteLine = callsitePrefix + "runner" + new string(' ', 320) + ";";
            ...
            """;
            TestProjectHelper.InsertIndexedFile(dbPath, "src/Session.cs", "csharp", source);
            var fileId = 0;
            using (var db = new DbContext(DbOpenIntent.WriteIndex, dbPath))
            {
                using var select = db.Connection.CreateCommand();
                select.CommandText = "SELECT id FROM files WHERE path = 'src/Session.cs'";
                fileId = Convert.ToInt32(select.ExecuteScalar());
                var writer = new DbWriter(db.Connection);
            ...
            AssertBodyExcerpt(
                ...
                expectedCallsiteContent: "runner",
                expectedCallsiteLine: 11,
                expectedCallsiteColumn: callsiteColumn,
                expectedCallsiteLength: "runner".Length,
            ...
            AssertBodyExcerpt(
                ...
                expectedCallsiteColumn: callsiteColumn,
                expectedCallsiteLength: "runner".Length,
                expectedCallsiteReferenceCount: 3,
                expectedCallsiteOmittedReferenceCount: 2,
                expectedCallsiteContentTruncated: true,
                forbiddenRecoveryPath: projectRoot);
            AssertBodyExcerpt(
            ...
            Assert.Contains("runner", textStdout);
            ...
            Assert.Equal(string.Empty, legacyCallerStderr);
            using (var legacyCallerDocument = ParseJsonOutput(legacyCallerStdout))
            {
                var legacyCaller = legacyCallerDocument.RootElement;
                Assert.Equal(12, legacyCaller.GetProperty("callsite_line").GetInt32());
                Assert.Equal("callsite_column_unavailable", legacyCaller.GetProperty("callsite_content_unavailable_reason").GetString());
                Assert.False(legacyCaller.TryGetProperty("callsite_column", out _));
                Assert.False(legacyCaller.TryGetProperty("callsite_content", out _));
            }
            ...
            using (var db = new DbContext(DbOpenIntent.WriteIndex, dbPath))
            {
                using var replaceChunks = db.Connection.CreateCommand();
                replaceChunks.CommandText = @"
                    ...
                    VALUES (@fileId, 0, 1, 10, @beforeGap),
                           (@fileId, 1, 12, @endLine, @afterGap);";
                replaceChunks.Parameters.AddWithValue("@fileId", fileId);
                replaceChunks.Parameters.AddWithValue("@beforeGap", string.Join('\n', sourceLines.Take(10)));
                replaceChunks.Parameters.AddWithValue("@endLine", sourceLines.Length);
                replaceChunks.Parameters.AddWithValue("@afterGap", string.Join('\n', sourceLines.Skip(11)));
                replaceChunks.ExecuteNonQuery();
            }

            var (gapExitCode, gapStdout, gapStderr) = CaptureConsole(() => QueryCommandRunner.RunReferences(
                ["Run", "--db", dbPath, "--json", "--body", "--snippet-lines", "3", "--limit", "1", "--exact-name"],
                _jsonOptions));

            Assert.Equal(CommandExitCodes.Success, gapExitCode);
            Assert.Equal(string.Empty, gapStderr);
            using (var gapDocument = ParseJsonOutput(gapStdout))
            {
                var gapResult = gapDocument.RootElement;
                Assert.Equal(11, gapResult.GetProperty("callsite_line").GetInt32());
                Assert.Equal(callsiteColumn, gapResult.GetProperty("callsite_column").GetInt32());
                Assert.Equal("runner".Length, gapResult.GetProperty("callsite_length").GetInt32());
                Assert.Equal("callsite_excerpt_unavailable", gapResult.GetProperty("callsite_content_unavailable_reason").GetString());
                Assert.False(gapResult.TryGetProperty("callsite_content", out _));
            }

            using (var db = new DbContext(DbOpenIntent.WriteIndex, dbPath))
            {
                using var clearColumn = db.Connection.CreateCommand();
                clearColumn.CommandText = @"
                    UPDATE symbol_references
                    SET column_number = NULL
                    WHERE file_id = @fileId AND symbol_name = 'Run' AND line = 11;";
                clearColumn.Parameters.AddWithValue("@fileId", fileId);
                clearColumn.ExecuteNonQuery();
            }

            var (nullableExitCode, nullableStdout, nullableStderr) = CaptureConsole(() => QueryCommandRunner.RunImpact(
                ["Run", "--db", dbPath, "--json", "--body", "--snippet-lines", "1", "--exact-name"],
                _jsonOptions));

            Assert.Equal(CommandExitCodes.Success, nullableExitCode);
            Assert.Equal(string.Empty, nullableStderr);
            using var nullableDocument = ParseJsonOutput(nullableStdout);
            var nullableCaller = nullableDocument.RootElement.GetProperty("callers")[0];
            Assert.Equal(11, nullableCaller.GetProperty("callsite_line").GetInt32());
            Assert.Equal("runner".Length, nullableCaller.GetProperty("callsite_length").GetInt32());
            Assert.Equal("callsite_column_unavailable", nullableCaller.GetProperty("callsite_content_unavailable_reason").GetString());
            Assert.False(nullableCaller.TryGetProperty("callsite_column", out _));
            Assert.False(nullableCaller.TryGetProperty("callsite_content", out _));
        }
        finally
        {
            TestProjectHelper.DeleteDirectory(projectRoot);
