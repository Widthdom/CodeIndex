...
{
    [Fact]
    public void Extract_Terraform_DottedAndRawObjectReferences_ReuseFixture()
    {
        ...
        Assert.Single(references.Where(reference =>
            ...
            && reference.ReferenceKind == "reference"));
        foreach (var rawObjectName in new[] { "instances", "max_size", "regions", "suffix" })
        {
            Assert.Single(references.Where(reference =>
                reference.SymbolName == rawObjectName
                && reference.ReferenceKind == "reference"));
        }
