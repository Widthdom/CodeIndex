...
{
    [Fact]
    public void Extract_CobolPerformAndCommonStatements_PreserveCallsReferencesAndContainers()
    {
        // Keep PERFORM range expansion and common statement targets in one program. NEXT-PARA
        // deliberately follows EXIT-PARA so it remains outside the HELPER-PARA THRU EXIT-PARA
        // range and cannot become an accidental range call.
        // PERFORM range と common statement target を1 program にまとめる。NEXT-PARA は
        // EXIT-PARA の後へ置き、THRU range による余分な call を防ぐ。
        ...
        var (symbols, references) = ExtractSymbolsAndReferences("cobol", content);
    ...
    public void Extract_CobolTargetStatements_ReuseSingleProgramFixture()
    {
        ...
        Assert.Equal(42, statements.Count(item => item.ReferenceKind == "reference"));
        Assert.Equal(4, statements.Count(item => item.ReferenceKind == "call"));
