...
public class ConsoleUiTests
{
    ...
    public void PrintCommandUsage_DocumentsJsonLineAndBatchContracts_Issue3916()
    {
        ...
        Assert.Contains("JSON is embedded as result, NDJSON as results", batchStdout);
        Assert.Contains("captured child output is capped", batchStdout);
    ...
    public void PrintUsage_WithoutBanner_HidesAsciiArtAndEasterEggFlags()
    {
        ...
        Assert.Contains("cdidx definition <query>|--query <query>|-- <query> [--db <path>] [--json] [--format <text|json|count|compact|csv|tsv|lsp|qf|sarif>] [--max-json-bytes <n>] [--verbose] [--limit <n>|--top <n>] [--lang <lang>] [--kind <kind>] [--visibility <v[,v]>] [--exclude-visibility <v[,v]>] [--path <glob>] [--exclude-path <glob>] [--exclude-tests] [--body] [--exact|--exact-name] [--count] [--group-partials] [--since <datetime>]", output);
