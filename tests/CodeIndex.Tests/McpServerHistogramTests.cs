...
public sealed class McpServerHistogramTests
{
    ...
    {
        var projectRoot = TestProjectHelper.CreateTempProject("cdidx_mcp_histogram");
        try
        {
            using var server = new McpServer(TestProjectHelper.ProjectPath(projectRoot, "codeindex.db"), "test");
