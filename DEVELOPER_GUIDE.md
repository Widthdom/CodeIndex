...
- **MCP file resource discovery** — `resources/templates/list` advertises `cdidx://file/{+path}` so a client that already knows an exact repository-relative path can construct a `resources/read` URI without paging the repository inventory. The reserved expansion preserves path separators; the read path still rejects absolute paths, traversal, backslashes, and encoded separators. `resources/list` accepts `path` as one string or at most 100 strings of at most 1024 characters and 128 wildcard operators each, using the same anchored directory/glob semantics as file queries, plus an exact normalized `lang` filter and `includeGenerated` (default `false`). Generated files also require `includeGenerated: true` for direct reads.
...
- The advertised capability surface includes `tools`, `resources`, and
  ...
  `cdidx://file/{+path}` for direct resolution of a known exact,
  repository-relative path. `resources/list` pages indexed files as
...
- **MCP ファイル resource discovery** — `resources/templates/list` は `cdidx://file/{+path}` を公開し、正確なリポジトリ相対 path が既知の client は全 inventory をページングせず `resources/read` URI を構築できる。reserved expansion は path separator を保持する一方、read path は absolute path、traversal、backslash、encoded separator を引き続き拒否する。`resources/list` は `path` に 1 文字列または各 1024 文字・wildcard operator 128 個以内かつ最大 100 件の文字列を受け付け、file query と同じ anchored directory / glob semantics を使うほか、正規化した完全一致の `lang` と `includeGenerated`（既定 `false`）も受け付ける。cursor は generation と canonical filter の両方に結び付き、ページ間の filter 変更は `-32602` / `resources_list_filters_changed` と `restart_required: true` を返す。generated file は discovery と direct read のどちらでも `includeGenerated: true` が必要である。
...
- `resources/templates/list` は正確な既知 path の直接解決用に `cdidx://file/{+path}` を公開する。`resources/list` の `path`、`lang`、`includeGenerated` filter は server-side で有界に適用され、継続 cursor は canonical filter と generation の両方に結び付く。generated file は既定で list と read から除外され、明示的な `includeGenerated: true` が必要になる。
