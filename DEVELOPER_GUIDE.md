...
The stable `hotspot_family_degraded_reason` codes are `hotspot_family_support_not_indexed`, `hotspot_family_metadata_stale`, `hotspot_family_disabled_at_index_time`, `partial_family_key_population`, and `hotspot_family_marker_fingerprint_incomplete`. `partial_family_key_population` means some indexed symbols still lack family keys and need a rebuild/restamp. The incomplete fingerprint code means marker fingerprint traversal hit safety caps during the last index run; narrow or ignore generated/vendor marker trees before rebuilding.
