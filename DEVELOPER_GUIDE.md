...
| Maintenance command precedence | `maintenance_guidance.recommended_command` preserves the existing vacuum-then-checkpoint precedence. When both are healthy and the trusted FTS write snapshot reaches its threshold, it returns `cdidx optimize --db <db>`; stale or unavailable FTS snapshots never select a mutation command. |
...
| maintenance command の優先順位 | `maintenance_guidance.recommended_command` は既存の vacuum、checkpoint の順序を維持します。両方が healthy で、信頼できる FTS write snapshot が threshold に達した場合は `cdidx optimize --db <db>` を返します。stale または unavailable な FTS snapshot では mutation command を選択しません。 |
