...
セミコロン形式の C# 位置 `record`、`record class`、`record struct` では、index 上の body range は record の宣言行から終端 `;` までの宣言全体を含み、軽量な64行の signature lookahead を超える宣言も対象です。このため `definition --body` は、`--fields` 投影時も含め、ほかの body と同じ bounded な行数 / byte truncation と recovery metadata を伴って宣言内容を返します。brace 形式の record は generic constraint または `allows ref struct` anti-constraint が `:` の後で改行されても brace に基づく body range を維持し、`N . Base` のように空白を含む修飾 base / constraint 型も宣言の継続として扱い、不完全な record が後続の型宣言や member 宣言を取り込むことはありません。parameterless record の `;` が次行の先頭にある場合も含め、セミコロン record の同一行 signature と参照 scope は終端で停止するため、後続 sibling の initializer は外側の型に帰属します。参照抽出は record owner ごとに収集した header boundary を再利用し、大きな複数行宣言で component reference ごとに header を再走査せず線形性を維持します。
