...
- **Portable trusted Git selection** — Git subprocesses select only validated known installation paths or the process-only `CDIDX_GIT_EXECUTABLE` absolute-path override; they never resolve an arbitrary `git` from `PATH`. Explicit overrides fail closed unless the target is a regular non-symlink/non-reparse/device file named `git` (`git.exe` on Windows). POSIX overrides must have an execute bit and no group/other write bits; Windows overrides must use `.exe`. CLI and MCP `status.git_executable` report the sanitized source, acceptance, stable rejection reason, owner-only-write result, Unix mode, and executable result, while accepted environment overrides also appear in `trust_overrides[]`. The owner-only-write result is a mode-policy diagnostic (absence of group/other write bits), not a claim that .NET resolved the numeric file owner.
- **Worktree-aware git exclude** — `.cdidx/` is auto-added to `.git/info/exclude`. In a worktree, `.git` is a file (not a directory), so the worktree root has no `.git/info/exclude`. `GitHelper.ResolveGitCommonDir()` chases references to find the shared `.git/`:
  ...
  Resolution: canonicalize and validate the `.git` directory/file boundary → read a regular `.git` file → parse `gitdir:` → validate its regular directory target → read a regular `commondir` file → resolve `../..` relative to `feature-branch/` dir (`feature-branch/` → `..` → `worktrees/` → `..` → `.git/`) → validate the common directory → write `info/exclude`. Symlink, reparse-point, device, wrong-entry-kind, or unreadable metadata entries fail closed before any metadata write.
...
- **portable な信頼済み Git 選択** — Git subprocess は検証済みの既知 installation path または process-only の絶対パス override `CDIDX_GIT_EXECUTABLE` だけを選び、`PATH` 上の任意の `git` は解決しない。明示 override は `git`（Windows は `git.exe`）という名前の regular かつ symlink / reparse point / device ではない file でなければ fail-closed になる。POSIX override は execute bit があり group / other write bit がないこと、Windows override は `.exe` であることを要求する。CLI / MCP の `status.git_executable` は sanitization 済み source、acceptance、stable な rejection reason、owner-only-write 判定、Unix mode、executable 判定を報告し、受理済み environment override は `trust_overrides[]` にも含まれる。owner-only-write は group / other write bit がないという mode policy の診断であり、.NET が数値 file owner を解決したという意味ではない。
- **worktree対応のgit exclude** — `.cdidx/`を`.git/info/exclude`に自動追加する。worktreeでは`.git`がディレクトリではなくファイルのため、worktreeルートには`.git/info/exclude`が存在しない。`GitHelper.ResolveGitCommonDir()`で参照を辿り共通`.git/`を見つける:
  ...
  解決手順: `.git` directory / file boundary を canonicalize・検証 → regular な `.git` file を読む → `gitdir:`を解析 → regular な directory target を検証 → regular な `commondir` file を読む → `../..`を`feature-branch/`ディレクトリ起点で解決（`feature-branch/` → `..` → `worktrees/` → `..` → `.git/`）→ common directory を検証 → `info/exclude`に書き込む。symlink、reparse point、device、entry kind 不一致、読取り不能な metadata entry は metadata write 前に fail-closed になる。
