...
- **Non-blocking overload.** A full or completed queue drops the new event and increments explicit drop counters. Producers do not wait for queue space, so a blocked sink cannot extend CLI or MCP response latency beyond bounded serialization and enqueue work.
...
- **Status and degradation.** Full MCP status publishes the enabled sink as `mcp_session.audit_log`; MCP ping/HTTP health mirrors it as `audit_log`. Alongside `enabled`, `path`, `include_values`, `max_bytes`, `bytes_written`, `disposed`, `queue_capacity`, and `queue_depth`, both surfaces expose `queued_record_count`, `written_record_count`, `shutdown_abandoned_record_count`, `shutdown_flush_timed_out`, `dropped_record_count`, `queue_full_drop_count`, `serialization_failure_count`, `write_failure_count`, `rotation_failure_count`, `rotation_cleanup_failure_count`, and `rotation_degraded`, plus optional `last_drop_reason` and `last_rotation_failure`. A positive dropped count, degraded rotation, or timed-out shutdown flush degrades the top-level MCP ping/health status.
...
- **ノンブロッキングな overload。** queue が満杯または complete 済みの場合、新しい event を drop して明示的な drop counter を増やします。producer は queue の空きを待たないため、sink が block しても、上限付き serialization と enqueue work を超えて CLI / MCP response の latency を延ばしません。
...
- **status / degradation。** MCP の full status は有効な sink を `mcp_session.audit_log` に公開し、MCP ping / HTTP health は同じ object を `audit_log` として返します。`enabled`、`path`、`include_values`、`max_bytes`、`bytes_written`、`disposed`、`queue_capacity`、`queue_depth` に加え、両 surface は `queued_record_count`、`written_record_count`、`shutdown_abandoned_record_count`、`shutdown_flush_timed_out`、`dropped_record_count`、`queue_full_drop_count`、`serialization_failure_count`、`write_failure_count`、`rotation_failure_count`、`rotation_cleanup_failure_count`、`rotation_degraded`、任意の `last_drop_reason`、`last_rotation_failure` を公開します。positive な dropped count、rotation degradation、または shutdown flush timeout は top-level MCP ping / health status を degraded にします。
