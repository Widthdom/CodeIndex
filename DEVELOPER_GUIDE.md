...
- **Bounded hotspot aggregation** — `DbWriter` maintains `hotspot_reference_counts` as compact per-file logical-reference totals. Hotspot readers aggregate and rank that table with indexable non-SQL, SQL exact, and SQL leaf joins before applying SQL `LIMIT`; they do not rematerialize the complete `symbol_references` graph on every limited query. Writable legacy databases create and backfill the table transactionally, while immutable legacy readers retain the raw-reference compatibility path.
...
- **bounded な hotspot 集約** — `DbWriter` は `hotspot_reference_counts` を file 単位の compact な logical-reference totals として維持する。hotspot reader は limit 付き query ごとに complete `symbol_references` graph を再 materialize せず、non-SQL、SQL exact、SQL leaf の索引可能な join でこの table を集約・ranking してから SQL `LIMIT` を適用する。writable な legacy database では table の作成と backfill を transaction 内で行い、immutable な legacy reader は raw-reference compatibility path を維持する。
