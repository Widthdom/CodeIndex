...
- **MCP index root identity.** `index` resolves the requested root canonically, captures its platform filesystem identity, traverses the canonical path, and revalidates the original mapping, root identity, current CWD/client-root containment, each traversed directory, and each file before content access. A change raises a bounded `permission_denied` tool error with `authorization_failure_reason`; successful/dry-run structured output and the matching audit record carry the same fixed-length opaque `checked_root_identity`. A containing repository root is used for ignore rules only when it remains authorized; otherwise discovery is confined to the requested project root.
...
- **MCP index の root identity。** `index` は要求された root を canonical に解決して platform filesystem identity を取得し、canonical path を走査します。元の mapping、root identity、現在の CWD/client-root containment、走査する各 directory、内容を読む各 file を再検証します。変化時は上限付きの `permission_denied` tool error と `authorization_failure_reason` を返し、成功時/dry-run の structured output と対応する audit record は同じ固定長 opaque `checked_root_identity` を保持します。包含 repository root は認可範囲内の場合だけ ignore rule に使い、範囲外なら discovery を要求 project root 内に限定します。
