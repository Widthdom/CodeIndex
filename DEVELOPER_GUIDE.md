...
3. **DB path resolution.** `DbPathResolver` computes
   ...
   `indexed_git_head` / `worktree_head_changed` unset on legacy DBs that
   have no stored root metadata, even when the explicit path itself
   looks like `.../.cdidx/codeindex.db`. When `indexed_git_head` is
