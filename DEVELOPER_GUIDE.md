...
Recipe materialization separates raw candidate-window exhaustion from origin classification coverage before display filtering. Observe rejected candidates too: an unknown origin cannot prove absence. Complete filtered results, including empty children, may expose authoritative source totals; stale/incomplete indexes, output omissions and identity checks remain independent baseline gates. Preserve explicit `origin_classification_incomplete`, `raw_candidate_window_exhausted`, `guard_filter_coverage_unverified`, `file_reject_coverage_unverified`, `result_kind_filter_coverage_unverified`, and `semantic_filter_coverage_unverified` reasons with appropriate manual-review or narrowing guidance. Baseline defaults are inserted only for user-absent limit options, preserving parser warnings for genuine duplicates and rightmost precedence. The v1 schema and effective scope/recipe fingerprints are unchanged.
...
Checkpoint manifest validation uses `ExtractorPluginRegistry.TryOpenSecureRegularFile` to reject final-component links and non-regular handles, including non-blocking POSIX FIFO rejection. The opened handle supplies both length and content; Windows sharing remains `ReadWrite | Delete`, so replacement after open does not redirect consumption, but concurrent content changes and ancestor replacement are not excluded. `DataDirectorySecurity.ReadTextWithinLimit(Stream, ...)` leaves ownership with its caller, consumes at most the 16 KiB budget plus one detection byte (the secure file stream may additionally prefetch up to 8 KiB), and decodes only accepted bytes with the existing BOM-aware decoder. Restore passes cancellation through reads and line validation. Preserve the existing manifest fields and sanitized diagnostics; no database migration is involved.
...
Named-query selectors (#5325) reuse `ApplySearchOutputSelection` independently
after each query's filtered/deduplicated fixed 10,000-candidate read. Preserve raw
candidate exhaustion through `SearchWithCandidateEvidence` before deduplication.
First-per-file then sampling precede per-query/remaining total limits; execute
later queries even after total-limit exhaustion to retain their accounting.
Optional `selection_accounting` uses the same source/selected/returned and stage
counts in rich, compact and projected objects; human output states their authority.
Byte admission remains all-or-error, with zero byte omissions in successful
documents. Reject selector cursors and accounting-free formats. This is CLI-only;
MCP's existing individual search/recipe contracts are unchanged.
...
- **Dependency evidence filters precede every graph bound** — `deps --resolution-state` / `--reference-kind` and MCP `resolutionStates` / `referenceKinds` select raw persisted observations before file-edge aggregation, ranking, ordinary/reverse limits, SCC candidate limits, and graph budgets. Values are normalized and bounded to 64 per dimension; OR applies within a dimension and AND between dimensions. Resolution filtering trusts only the current reference-identity contract: missing, stale, NULL, and unknown values map to `unavailable`, never `unresolved`. Kind filtering reuses the canonical logical/raw mapping, including canonical `subscribe` expansion. Query context and cycle cursor fingerprints carry both normalized selectors, while graph readiness warnings remain unchanged because filtering cannot establish whole-program completeness (#5280).
...
残りの全体上限の順に適用し、全体上限到達後も後続クエリを評価して件数を残します。
任意の `selection_accounting` は rich / compact / 投影 object で同じ source / selected /
returned と段階別件数を保持し、human 出力でも確定値か下限かを明示します。
バイト上限は文書全体を採用するかエラーを返す方式で、成功文書のバイト省略数は 0 です。
選択時の cursor と件数情報を保持できない形式は拒否します。CLI の拡張であり、
MCP の既存の単一検索・レシピ契約は変更しません。
...
監査の再開では `DbReader.Search` から重複除去前の候補枠終了を受け取り、上限付き guard／ranking の情報も含めます。JSON の query detail 上限で隠れる行は、再開位置を計算する前に除去してください。NDJSON の行の出力可否は detail metadata とは独立です。子 query が完了すると token や再実行コマンドが不連続に消えるため、JSON/NDJSON の byte 判定では query 完了位置を個別に確認します。
...
レシピの結果生成では、生の候補枠の終了と、表示フィルター適用前の origin 分類の完全性を分離します。除外する候補も確認し、不明な origin を不在の証明に使わないでください。0件の子クエリを含む完全なフィルター結果は件数を確定できますが、索引の鮮度・完全性、出力省略、証拠の識別は独立した baseline 判定条件です。`origin_classification_incomplete`、`raw_candidate_window_exhausted`、`guard_filter_coverage_unverified`、`file_reject_coverage_unverified`、`result_kind_filter_coverage_unverified`、`semantic_filter_coverage_unverified` と、それぞれに適した手動レビュー・範囲縮小の案内を維持してください。baseline の上限既定値はユーザー指定がない場合だけ追加し、本当の重複に対する parser の警告と最後の値の優先を維持します。v1 スキーマと実効範囲・レシピの指紋は変更しません。
...
チェックポイントの manifest 検証では `ExtractorPluginRegistry.TryOpenSecureRegularFile` で末尾のリンクと通常ファイル以外のハンドルを拒否し、POSIX の FIFO も非ブロッキングで拒否します。サイズと内容は同じハンドルから取得します。Windows の共有指定は `ReadWrite | Delete` なので、open 後の差し替えで読み取り先は変わりませんが、内容の同時変更や親ディレクトリの差し替えは排除しません。`DataDirectorySecurity.ReadTextWithinLimit(Stream, ...)` は呼び出し元に所有権を残し、16 KiB の上限と検出用 1 バイトまでを消費します（安全に開いたファイルストリームは追加で最大 8 KiB を先読みする場合があります）。受理したバイトだけを既存の BOM 対応デコーダーで変換します。復元時は読み取りと行検証にキャンセルを伝播します。既存の manifest フィールドとサニタイズ済み診断を維持し、DB の移行は不要です。
...
- **依存 evidence filter はすべての graph 上限より前に適用** — `deps --resolution-state` / `--reference-kind` と MCP `resolutionStates` / `referenceKinds` は、file edge 集約、ranking、通常 / reverse の limit、SCC candidate limit、graph budget より前に raw の保存済み observation を選択します。値は正規化し、dimension ごとに最大 64 件に制限します。同じ dimension 内は OR、dimension 間は AND です。resolution filter が信頼するのは current な reference-identity contract だけで、欠落・stale・NULL・未知値は `unresolved` ではなく `unavailable` に分類します。kind filter は canonical `subscribe` の展開を含む既存の logical / raw mapping を再利用します。query context と cycle cursor fingerprint は正規化済み selector の両方を保持し、filter は whole-program completeness を証明しないため graph readiness warning は維持します（#5280）。
