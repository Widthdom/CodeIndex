...
- **MCP サーバー instructions** — `initialize` response の初回案内は UTF-8 で 2 KiB 以内に保つ。bounded な `tools/list` discovery と完全定義の on-demand 取得を説明し、有効な tool だけを案内し、完全 catalog の説明を重複させず詳細 workflow を `prompts/list` と `prompts/get` へ誘導する。
