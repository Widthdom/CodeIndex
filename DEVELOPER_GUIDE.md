...
and typed tool errors. Success variants require tool-specific discriminator fields,
typed errors carry the same root `api_version`, and open compatibility fields remain
bounded by property, array-item, and string-length limits. Keep field names and nesting
...
pagination / truncation、warning、型付き tool error を表します。success variant は tool 固有の
discriminator field を必須とし、typed error も同じ root `api_version` を持ち、互換性のための open field は
property 数、array item 数、string 長の上限で bounded に保ちます。field 名と nesting を実際の structured
