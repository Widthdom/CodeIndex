...
| Open intent | Every `DbContext` caller declares `QueryOnly`, `WriteIndex`, `Migration`, or `Repair`. `QueryOnly` uses unpooled connections and skips persistent pragmas, migrations, metadata writes, and repair work. For a checkpointed WAL database it uses an immutable read-only URI so SQLite cannot create or touch source sidecars. For a non-empty WAL it copies a stable main/WAL pair to a private temporary snapshot and reads committed WAL content there; if the source changes throughout the bounded copy attempts, it refuses the open instead of returning stale data or modifying source `-wal` / `-shm`. `RepairIncompleteBatchReadiness` is guarded by `Repair` intent. |
...
| open intent | すべての `DbContext` caller は `QueryOnly`、`WriteIndex`、`Migration`、`Repair` のいずれかを宣言します。`QueryOnly` は unpooled connection を使い、永続的な pragma、migration、metadata write、repair を skip します。checkpoint 済みの WAL database では immutable read-only URI を使い、SQLite が source sidecar を作成・更新できないようにします。non-empty WAL では安定した main/WAL pair を private temporary snapshot へ copy して committed WAL content をそこで読みます。bounded copy の間に source が変化し続けた場合は、stale data を返したり source `-wal` / `-shm` を変更したりせず open を拒否します。`RepairIncompleteBatchReadiness` は `Repair` intent の場合に限り実行できます。 |
