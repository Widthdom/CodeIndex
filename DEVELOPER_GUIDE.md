...
| Temp-file replacement policy (#3996) | The normalizer writes each replacement archive to a unique same-directory `.normalize-tmp.<guid>` sidecar, fsyncs the completed temp file, atomically moves it over the package, and flushes the parent directory on Unix. A stale legacy `.normalize-tmp` sidecar is deleted only after the tool can open it exclusively, so preexisting temp files do not block normal rewrites or race active normalizers. |
...
| temp-file replacement policy (#3996) | normalizer は replacement archive を同じ directory の一意な `.normalize-tmp.<guid>` sidecar に書き込み、完成した temp file を fsync し、package に atomic move してから Unix では parent directory も flush します。古い legacy `.normalize-tmp` sidecar は tool が排他的に open できた場合だけ削除するため、既存 temp file が通常の rewrite を妨げたり、実行中の normalizer と競合したりしません。 |
