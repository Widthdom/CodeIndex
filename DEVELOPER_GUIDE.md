...
- The advertised capability surface includes only server-provided `tools`,
  ...
  drive-prefixed, NUL-containing, or `..`-traversing values with JSON-RPC
  `-32602` before constructing the prompt. Accepted workspace-relative paths
  preserve spaces and Unicode while normalizing backslashes to `/`. `logging` advertises
