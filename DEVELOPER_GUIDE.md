...
| Generated-code filtering metadata | Discovery `query_context` always reports `include_generated` and `generated_code_policy`. The `files --count --json` and every JSON `map` summary (including `issue-drafts`) also report `generated_file_count_excluded`, `generated_file_count_excluded_authoritative`, and `generated_file_filter_available`. The excluded count is `0` when generated files are included. For a legacy DB without `files.generated`, the policy is `unavailable`, the count is `null`, and the authoritative/available flags are `false` rather than claiming that an unavailable filter ran. |
...
| Contract area | Rule |
|---|---|
| DTOs carrying `api_version` | Every top-level CLI/MCP JSON success or failure DTO carries an `api_version` string stamped from `JsonOutputContract.ApiVersion`. Primary query DTOs include `StatusResult`, `RepoMapResult`, `SymbolAnalysisResult`, `ImpactAnalysisResult`, `OutlineResult`, `FileExcerptResult`, `CompactSearchResult`, `SymbolResult`, `DefinitionResult`, `UnusedSymbolResult`, `ReferenceResult`, `CallerResult`, `CalleeResult`, `FileResult`, and `FileFindResult`. Audited utility DTOs implement the shared `IVersionedJsonResult` contract, including command errors, upgrade checks, database restore/backup maintenance, diff results, hooks, workspace/config, language/version inventory, and index run/watch results. |
...
| generated-code filtering metadata | discovery の `query_context` は常に `include_generated` と `generated_code_policy` を返します。`files --count --json` と `issue-drafts` を含むすべての JSON `map` summary は、`generated_file_count_excluded`、`generated_file_count_excluded_authoritative`、`generated_file_filter_available` も返します。generated file を含める場合、除外数は `0` です。`files.generated` が無い legacy DB では、未実行の filter を実行済みと誤認させないよう、policy は `unavailable`、count は `null`、authoritative / available flag は `false` になります。 |
