...
`exit_code`, and captured child `stderr`. Successful child stdout that is one
complete JSON document is embedded as typed `result`; successful NDJSON is
embedded as a typed `results` array. Non-JSON stdout and output from failed
...
output must not be written directly beside batch metadata in this mode. Captured
child stdout/stderr shares a 1,048,576-character batch budget; an item that
...
`command_failures`, and `exit_code`, and publishes `captured_output_chars` plus
`captured_output_char_limit` for empty-input, failure, and budget accounting.
...
`line`、`command`、`arguments`、`exit_code`、捕捉した child `stderr` を含める。成功した child
stdout が完全な JSON document なら型付き `result`、NDJSON なら型付き `results` array として
埋め込む。非 JSON stdout と失敗 command の出力は診断を失わないよう raw `stdout` text のまま
保持する。malformed line や入力上限超過 line は `record: "batch_error"` と `error` object を使う。
この mode では child output を batch metadata と並べて直接出力してはならない。捕捉する child
stdout / stderr は batch 全体で 1,048,576 文字の budget を共有し、使い切った item も
...
`command_failures`、`exit_code`、`captured_output_chars`、`captured_output_char_limit` を保持する。
