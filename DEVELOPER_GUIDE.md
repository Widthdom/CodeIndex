...
- **C# switch-pattern/call boundary** — Positional-pattern suppression may scan a following physical line for an arm arrow only when the candidate's closing parenthesis is followed solely by trivia. A comma or any other token ends that candidate, so calls inside lambdas returned from switch-expression arms remain call references attributed to the enclosing callable while genuine pattern heads remain excluded. C# extractor contract v10 refreshes stale C# rows under this rule (#5085).
...
- **C# switch pattern と call の境界** — positional pattern の抑止処理が次の物理行に arm の矢印を探してよいのは、候補の閉じ括弧以降が trivia だけの場合です。カンマなど別の token があればその候補はそこで終了するため、switch expression の arm が返す lambda 内の call は enclosing callable に属する call reference として保持し、本物の pattern head だけを除外します。C# extractor contract v10 はこの規則で古い C# row を更新します（#5085）。
