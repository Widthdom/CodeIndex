...
| Authoritative-fresh raw insert scope | After the empty-database CLI path revalidates its authoritative-fresh claim inside the caller-owned transaction, only the extraction pipeline's DONE-only chunk, symbol, new-file issue, and atomic fresh-reference INSERTs may bind and execute through SQLitePCLRaw on the provider-owned connection handle. The scope preserves the existing 32-parameter statement boundaries, exact tail shapes, batch hooks, row-skip replay, and outer transaction atomicity; a 16-entry LRU covers the 14 production row shapes. The synchronous full-scan persistence consumer and `DbWriter` transaction-owner check keep the non-thread-safe cache single-owner. Every lease resets and clears bindings, errors retain the original step result, cancellation maps SQLite interrupt to `OperationCanceledException`, and all cached statements are finalized before graph/index/FTS work. File and reference-line `RETURNING` materialization, replacement, incremental, rebuild, symbols-only, fresh-claim race fallback, MCP, and public writer paths remain on Microsoft.Data.Sqlite. |
...
| authoritative-fresh raw insert scope | empty-database CLI経路がcaller-owned transaction内でauthoritative-fresh claimを再検証した後に限り、extraction pipelineのDONE-onlyなchunk、symbol、new-file issue、atomic fresh-reference INSERTをprovider所有connection handle上のSQLitePCLRawでbind / executeします。scopeは既存の32 parameter statement境界、正確なtail形状、batch hook、row-skip replay、outer transaction atomicityを維持し、16-entry LRUで本番14形状を保持します。同期的なfull-scan persistence consumerと`DbWriter`のtransaction owner検査により、非thread-safe cacheはsingle-ownerのままです。各leaseはresetとbinding clearを行い、error時は元のstep結果を保持し、SQLite interruptを`OperationCanceledException`へ変換し、graph / index / FTS処理より前に全cached statementをfinalizeします。file / reference-lineの`RETURNING` materialization、replacement、incremental、rebuild、symbols-only、fresh-claim race fallback、MCP、public writer経路はMicrosoft.Data.Sqliteを維持します。 |
