...
| CODEOWNERS | Path-aware detection recognizes only `.github/CODEOWNERS`, repository-root `CODEOWNERS`, and `docs/CODEOWNERS`. A dedicated bounded line parser emits ordered ownership `rule` symbols and child owner `property` symbols, including ownerless rules, with limits of 4,096 characters per line, 128 owners per rule, and 256 characters per owner. Invalid input produces category-deduplicated extraction diagnostics. No reference extractor is registered because owner authorization and pattern-to-path resolution are outside the syntax-only contract. |
...
| CODEOWNERS | path-aware detection は `.github/CODEOWNERS`、repository root の `CODEOWNERS`、`docs/CODEOWNERS` だけを認識します。専用の上限付き line parser が順序付き ownership `rule` symbol と child owner `property` symbol を出力し、ownerless rule も保持します。上限は1行4,096文字、1 rule 128 owner、1 owner 256文字です。不正 input は category ごとに重複を除いた extraction diagnostic にします。owner authorization と pattern から path への解決は syntax-only contract の範囲外なので、reference extractor は登録しません。 |
