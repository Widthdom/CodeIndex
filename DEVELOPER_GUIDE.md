...
- The `initialize` response returns `protocolVersion`, `capabilities`,
  ...
  That handshake-triggered refresh is coalesced to one task per session, and
  transport teardown drains the task so repeated initialized notifications
  cannot accumulate detached client requests.
