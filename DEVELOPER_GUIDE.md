...
HDL reference extraction は意図的に構文ベースとし、公開済みの reference lookup 上限に従います。comment と string を mask し、対応するローカル宣言がなくても hierarchy/package/architecture edge を出力しますが、signal、type、function reference は現在のファイルで既知の symbol に限定します。generate、preprocessor macro、parameterized hierarchy、signal data flow の elaboration は行いません。
