...
| Generated-code filtering metadata | Discovery `query_context` always reports `include_generated` and `generated_code_policy`. The `files --count --json` and JSON `map` summaries also report `generated_file_count_excluded`; it is `0` when generated files are included. |
...
| generated-code filtering metadata | discovery の `query_context` は常に `include_generated` と `generated_code_policy` を返します。`files --count --json` と JSON `map` summary は `generated_file_count_excluded` も返し、generated file を含める場合は `0` です。 |
