            ...
            sql += exact
                ? rustQualifiedParts.QualifiedPath != null
                    ...
                    : _foldReady
                        ...
                        : allowLeafFallback
                            ? $" AND ({BuildExactPrimarySymbolNameMatchSql("@query", false, normalizedQuery, lang)} OR {markdownAnchorExactClause} OR (f.lang = 'sql' AND ((sql_segment_count(s.name) = @querySegmentCount AND sql_normalize_name(s.name) = @queryNormalized COLLATE NOCASE) OR sql_leaf_name(s.name) = @queryLeaf COLLATE NOCASE)))"
                            : $" AND ({BuildExactPrimarySymbolNameMatchSql("@query", false, normalizedQuery, lang)} OR {markdownAnchorExactClause} OR (f.lang = 'sql' AND sql_segment_count(s.name) = @querySegmentCount AND sql_normalize_name(s.name) = @queryNormalized COLLATE NOCASE){(qualifiedSymbolClause != null ? $" OR {qualifiedSymbolClause}" : string.Empty)})"
