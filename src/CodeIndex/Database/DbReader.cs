...
public partial class DbReader : IDisposable
{
    ...
    private HashSet<string> LoadIndexedHotspotFamilyLanguages()
    {
        ...
        for (int i = 0; i < hotspotFamilyLangs.Count; i++)
            SqliteCommandPolicy.Add(cmd, $"@hfl{i}", hotspotFamilyLangs[i]);
    ...
    internal static string BuildPathLikePattern(string input)
    {
        ...
        input = NormalizePathLikePatternInput(input);
    ...
    internal static string BuildPathSubtreeLikePattern(string input)
        => BuildPathLikePattern(input) + "/%";

    private static string NormalizePathLikePatternInput(string input)
    {
    ...
    private string BuildGraphSupportedLanguagePredicate(SqliteCommand cmd, string fileAlias, string parameterPrefix)
    {
        ...
        for (int i = 0; i < supportedLanguages.Count; i++)
        {
            ...
            SqliteCommandPolicy.Add(cmd, parameterName, supportedLanguages[i]);
        }
    ...
    internal bool AnyFilePathHasLanguage(IEnumerable<string> paths, string lang)
    {
        ...
        for (int i = 0; i < distinctPaths.Count; i++)
        {
            ...
            SqliteCommandPolicy.Add(cmd, parameterName, distinctPaths[i]);
        }
        ...
        cmd.CommandText = $"""
            ...
            LIMIT 1
            """;
        SqliteCommandPolicy.Add(cmd, "@sqlPathLang", lang);
        return cmd.ExecuteScalar() != null;
    }
    ...
    public List<FileResult> ListFiles(string? query = null, int limit = 20, string? lang = null, IReadOnlyList<string>? pathPatterns = null, IReadOnlyList<string>? excludePathPatterns = null, bool excludeTests = false, DateTime? since = null, bool orderBySize = false, int offset = 0, IReadOnlyList<string>? requiredPathPatterns = null)
    {
        ...
        cmd.CommandText = sql;
        if (query != null)
            SqliteCommandPolicy.Add(cmd, "@query", $"%{EscapeLikeQuery(query)}%");
        if (lang != null)
    ...
    internal static void AppendPathFilters(ref string sql, IReadOnlyList<string>? pathPatterns, IReadOnlyList<string>? excludePathPatterns, bool excludeTests, bool applyGeneratedFilter = true)
    {
        ...
        if (ExactFilePath.Value != null)
            sql += " AND f.path = @exactFilePath COLLATE BINARY";
        ...
        if (pathPatterns != null && pathPatterns.Count > 0)
        {
            ...
            for (int i = 0; i < pathPatterns.Count; i++)
                ors.Add(BuildPathFilterPredicate("f", "pathPattern", i, pathPatterns[i]));
        ...
        if (excludePathPatterns != null)
        {
            for (int i = 0; i < excludePathPatterns.Count; i++)
                sql += $" AND NOT {BuildPathFilterPredicate("f", "excludePathPattern", i, excludePathPatterns[i])}";
        }
    ...
    {
        EnsurePathFilterParameterBudget(pathPatterns, excludePathPatterns);
        if (ExactFilePath.Value is { } exactFilePath)
            SqliteCommandPolicy.AddText(cmd, "@exactFilePath", exactFilePath);
    ...
    internal static void AppendAdditionalPathIncludeFilters(ref string sql, IReadOnlyList<string>? pathPatterns, string parameterPrefix)
    {
        ...
        SqliteDynamicSql.EnsureParameterBudget(CountPathFilterParameters(pathPatterns), "additional path include filters");
        var ors = new List<string>(pathPatterns.Count);
        for (int i = 0; i < pathPatterns.Count; i++)
            ors.Add(BuildPathFilterPredicate("f", parameterPrefix, i, pathPatterns[i]));
        sql += " AND (" + string.Join(" OR ", ors) + ")";
    }
    ...
    internal static void AddPathIncludeFilterParameters(SqliteCommand cmd, IReadOnlyList<string>? pathPatterns, string parameterPrefix)
    {
        ...
        AddPathFilterParameterSet(cmd, parameterPrefix, pathPatterns);
    }
    ...
    internal static string BuildPathFiltersSql(string fileAlias, IReadOnlyList<string>? pathPatterns, IReadOnlyList<string>? excludePathPatterns, bool excludeTests)
    {
        ...
        if (ExactFilePath.Value != null)
            sql += $" AND {fileAlias}.path = @exactFilePath COLLATE BINARY";
        ...
        if (pathPatterns != null && pathPatterns.Count > 0)
        {
            ...
            var ors = new List<string>(pathPatterns.Count);
            for (int i = 0; i < pathPatterns.Count; i++)
                ors.Add(BuildPathFilterPredicate(fileAlias, "pathPattern", i, pathPatterns[i]));
            sql += " AND (" + string.Join(" OR ", ors) + ")";
        }
        ...
        {
            for (int i = 0; i < excludePathPatterns.Count; i++)
                sql += $" AND NOT {BuildPathFilterPredicate(fileAlias, "excludePathPattern", i, excludePathPatterns[i])}";
        }
