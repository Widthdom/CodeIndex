...
public static partial class ConsoleUi
{
    ...
    [
        ...
        ("audit", "cdidx audit (<recipe|recipe/query>|--all) [search filters] [--json[=ndjson]] [--format <text|json|count|compact|issue-drafts>] [--summary-only] [--summary-level <top|detailed> (all only)] [--partition-plan [--plan-cursor <token>] (all only)] [--partition <token> (all only)] [--limit <n>] [--total-limit <n>] [--continuation <token> (all only)] [--progress (all only)] [--no-progress] [--quiet] [--allow-partial] [--results-only] [--search-fields <csv>] [--first-per-file] [--sample <n>] [--max-json-bytes <n>] [--snippet-lines <n>]"),
        ...
        ("deps", "cdidx deps [--db <path>] [--json] [--format <dot|graphml|json-graph|edgelist>] [--summary-only] [--max-json-bytes <n>] [--verbose] [--limit <n>|--top <n>] [--cursor <cursor>] [--graph-budget <n>] [--lang <lang>] [--path <glob>] [--exclude-path <glob>] [--exclude-tests] [--reverse] [--cycles] [--group-partial-types] [--node-mappings] [--cycle-node <id>] [--node-generation <token>] [--mapping-cursor <token>] [--suppress-noise] [--resolution-state <state[,state]>] [--reference-kind <kind[,kind]>] [--symbol <name>] [--symbol-family <prefix>]"),
        ("unused", "cdidx unused [--analysis-timeout-ms <1..600000>] [--progress] [--db <path>] [--json] [--compact] [--summary-only] [--max-json-bytes <n>] [--verbose] [--limit <n>|--top <n>] [--cursor <next_cursor>] [--audit-scope <source|production-and-tooling|all>] [--kind <kind>] [--bucket <bucket>] [--min-confidence <medium|low>|--confidence <medium|low>] [--actionable] [--all] [--visibility <v[,v]>] [--exclude-visibility <v[,v]>] [--lang <lang>] [--path <glob>] [--exclude-path <glob>] [--exclude-tests] [--count] [--by-bucket]"),
        ...
        ("suggestions-link", "cdidx suggestions link <id> --repo <owner/name> --issue <number-or-url> [--actor <name>] [--reason <text>] [--db <path>] [--json]"),
    ...
    [
        ...
        ("suggestions-link", "Locally associates an existing github.com issue; repeat for multiple suggestions consolidated into one issue. No network access or remote issue creation occurs, even offline; existence, issue-vs-PR identity for numeric input, and remote state remain unverified."),
        ("suggestions-link", "Requires a matching owner/name repository and a positive issue number or https://github.com/owner/name/issues/number URL. Other hosts, PR URLs, query strings and fragments are rejected. Same-identity repeats preserve audit timestamps; conflicting reassociation is rejected."),
        ("suggestions-link", "Stores manual_external provenance and an audit timestamp atomically. Drafts become submitted_pending_triage; other lifecycle states and real submission/sync/resolution evidence are preserved. Linked records are excluded by --status unsubmitted and cannot be submitted again."),
        ("suggestions-link", "Example: `cdidx suggestions link <id> --repo Widthdom/CodeIndex --issue 5350 --actor maintainer --json`."),
