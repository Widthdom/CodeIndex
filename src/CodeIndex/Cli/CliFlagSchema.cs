...
internal static class CliFlagSchema
{
    ...
    private static IReadOnlyList<CliFlag> BuildAll()
    {
        return new List<CliFlag>
        {
            ...
            new() { Name = "--open-issues", ValuePlaceholder = "<path|github|github:owner/name>", Description = "Search/Map issue-drafts: preflight duplicate issues from JSON or GitHub", Commands = Set("search", "map") },
            new() { Name = "--repo", ValuePlaceholder = "<owner/name>", Description = "Repository for GitHub duplicate preflight", Commands = Set("search", "map") },
            new() { Name = "--issue-state", ValuePlaceholder = "<open|closed|all>", Description = "GitHub duplicate preflight issue history to inspect", Commands = Set("search", "map") },
