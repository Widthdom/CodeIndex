...
internal static class CliFlagSchema
{
    ...
    private static IReadOnlyList<CliFlag> BuildAll()
    {
        return new List<CliFlag>
        {
            ...
            new() { Name = "--unique", ValuePlaceholder = "<path|file|symbol|origin|return-type>", Description = "Search recipes: emit unique aggregation rows", Commands = Set("search") },
            new() { Name = "--count-by", ValuePlaceholder = "<path|file|symbol|origin|return-type>", Description = "Search recipes: count matches grouped by path/file, symbol, origin, or enclosing return type", Commands = Set("search") },
            ...
            new() { Name = "--group-by", ValuePlaceholder = "<file|symbol|origin|return-type|statement>", Description = "Search: group --count rows by file, symbol, origin, or enclosing return type; hotspots: symbol/file grouping, with statement only for --lang sql", Commands = Set("hotspots", "search") },
