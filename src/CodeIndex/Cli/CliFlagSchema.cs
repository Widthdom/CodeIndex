...
internal static class CliFlagSchema
{
    ...
    private static readonly string[] AllowPartialCommands = ["search", "symbols", "files"];
    ...
    private static IReadOnlyList<CliFlag> BuildAll()
    {
        return new List<CliFlag>
        {
            ...
            new() { Name = "--format", ValuePlaceholder = "<text|json|count|compact|grouped|csv|tsv|lsp|qf|sarif|dot|graphml|json-graph|edgelist|markdown|issue-drafts>", Description = "Standard output format for token budgets, editor integrations, and CI; supported values vary by command, and search recipes/map/suggestions export also accept issue-drafts", Commands = Set(FormatCommands) },
            ...
            new() { Name = "--by-bucket", Description = "Unused: include per-bucket grouped result arrays in JSON output, or count/representative summaries with --compact", Commands = Set(ByBucketCommands) },
            ...
            new() { Name = "--line-scan-limit", ValuePlaceholder = "<n>", Description = "Find: override the --all indexed-line scan cap", Commands = Set("find") },
            ...
            new() { Name = "--allow-partial", Description = "Return exit code 0 instead of 11 when a byte cap produces explicit partial-result metadata", Commands = Set(AllowPartialCommands) },
            ...
            new() { Name = "--sections", ValuePlaceholder = "<summary,tree,languages,hotspots,metrics|list>", Description = "Map: comma-separated response sections to include, or list to discover sections", Commands = Set(MapSectionCommands) },
            ...
            new() { Name = "--open-issues", ValuePlaceholder = "<path|github|github:owner/name>", Description = "Preflight issue drafts against issue JSON or GitHub issues", Commands = Set("search", "map", "suggestions") },
            new() { Name = "--repo", ValuePlaceholder = "<owner/name>", Description = "Issue-drafts: GitHub repository for --open-issues github", Commands = Set("search", "map", "suggestions") },
            new() { Name = "--issue-state", ValuePlaceholder = "<open|closed|all>", Description = "Issue-drafts: GitHub issue history state to inspect", Commands = Set("search", "map", "suggestions") },
