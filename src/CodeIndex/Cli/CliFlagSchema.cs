...
internal static class CliFlagSchema
{
    ...
    private static IReadOnlyList<CliFlag> BuildAll()
    {
        return new List<CliFlag>
        {
            ...
            new() { Name = "--all-cycle-nodes", Description = $"Deps cycles: expand every SCC path instead of the default {QueryCommandRunner.DefaultDependencyCycleNodeLimit}-node sample", PrimaryCommands = Set(DependencyCycleCommands) },
