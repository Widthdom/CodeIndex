...
public partial class McpServer
{
            ...
            CreateToolDefinition(
                ...
                new JsonObject
                {
                    ...
                    ["properties"] = new JsonObject
                    {
                        ...
                        ["auditScope"] = new JsonObject { ["type"] = "string", ["enum"] = new JsonArray { "source", "production-and-tooling", "all" }, ["description"] = "Recipe runs only: source applies production-code defaults; production-and-tooling adds indexed automation, installers, and repository tooling while excluding tests, fixtures, docs, and recipe definitions; all searches every indexed path unless other filters exclude it." },
                        ...
                        ["guardScope"] = new JsonObject { ["type"] = "string", ["enum"] = new JsonArray { "window", "same-line", "same-symbol" }, ["description"] = "Evaluate guards in a line window, on the same line, or in a C# symbol-bounded window. Same-symbol excludes the focus line and fails explicitly for unavailable ranges, anonymous functions and ambiguous expression bodies.", ["default"] = "window" },
    ...
    private static JsonObject[] CreateWorkspaceInspectionTools()
    {
        ...
        [
            CreateToolDefinition(
                "status",
                "Get database statistics, readiness state, and optional CLI-style freshness checks. Use `explainField` for a static CLI status-field explanation without reading runtime values; `format` and `maxBytes` bound that explanation. Existing `explain` modes provide aggregate runtime diagnostics. / DB統計、readiness 状態、必要に応じて CLI 風の freshness check を取得。`explainField` は実行時の値を読まず CLI と共通のフィールド説明を返し、`format` と `maxBytes` で説明出力を制限する。既存の `explain` は実行時診断をまとめて返す。",
                new JsonObject
                {
                    ["type"] = "object",
                    ["properties"] = new JsonObject
                    {
                        ...
                        ["explainField"] = new JsonObject { ["type"] = "string", ["description"] = "Static CLI status-field explanation from StatusResult serializer metadata, including dot-separated paths (at most 240 characters and 4 segments), case-insensitive names and CLI label aliases. Examples: index_complete, db_pragma_settings.busy_timeout_ms. Use only with format/maxBytes; incompatible with runtime status options and fields. Does not open the database." },
                        ["maxBytes"] = new JsonObject { ["type"] = "integer", ["minimum"] = 1, ["description"] = "Requires explainField. Bound successful structuredContent UTF-8 bytes plus one newline. Selects the required compact explanation schema; server/transport limits also bound the entire response. E028_RESPONSE_BUDGET_TOO_SMALL reports measured minimum sizes and retry guidance when the explanation cannot fit; the terminal error may exceed maxBytes." },
            ...
            CreateToolDefinition(
                ...
                new JsonObject
                {
                    ...
                    ["properties"] = new JsonObject
                    {
                        ...
                        ["resolutionStates"] = DependencyEvidenceValuesSchema(DependencyEvidenceFilter.ResolutionStates, "Select current persisted resolution states before aggregation and graph budgets. Missing, stale, NULL, or unknown evidence is unavailable; resolved_group is separate from resolved. Omit for all states."),
                        ["referenceKinds"] = DependencyEvidenceValuesSchema(DependencyEvidenceFilter.ReferenceKinds, "Select reference kinds before aggregation and graph budgets. Canonical subscribe includes raw event variants; explicit raw kinds match exactly. Omit for all kinds."),
                        ["suppressNoise"] = new JsonObject { ["type"] = "boolean", ["description"] = "For cycle analysis, suppress documented non-authoritative Markdown and qualified C# evidence while reporting removed counts and reasons. / cycle 解析で、文書化された非 authoritative な Markdown および修飾付き C# evidence を抑制し、除外件数と理由を報告する。", ["default"] = false },
                        ["summaryOnly"] = new JsonObject { ["type"] = "boolean", ["description"] = "Return bounded SCC metrics and node samples without the regular cycles collection; incompatible with format=json-graph. / 通常の cycles collection を省略し、上限付き SCC metrics と node sample を返す。format=json-graph とは併用不可。", ["default"] = false },
                        ["groupPartialTypes"] = new JsonObject { ["type"] = "boolean", ["description"] = "Group authoritative C# partial-type families before SCC analysis; requires cycles=true. / cycles=true で C# partial 型の確実な所属情報を用いて SCC 解析前にグループ化する。", ["default"] = false },
                        ["includeAllCycleNodes"] = new JsonObject { ["type"] = "boolean", ["description"] = "Expand every SCC node ID (file paths in raw mode); declaration mapping limits remain independent. / SCC の全ノード ID（生ファイルモードではパス）を展開する。宣言マッピングの上限は独立して維持する。", ["default"] = false },
                        ["nodeMappings"] = new JsonObject { ["type"] = "boolean", ["description"] = "Page all indexed declaration mappings independently of cycle membership; requires cycles=true and groupPartialTypes=true, and excludes graph filters. At most 40 nodes and 20 paths per node. / 循環への所属とは独立して全索引内の宣言をページ取得する。cycles=true と groupPartialTypes=true が必須でグラフフィルターは併用不可。最大40ノード・各20パス。" },
                        ["cycleNode"] = new JsonObject { ["type"] = "string", ["maxLength"] = DependencyCycleNavigation.MaxNodeLength, ["description"] = "Resolve one emitted node ID; requires nodeMappings and its nodeGeneration. / nodeMappings と同じ応答の nodeGeneration を指定し、返されたノード ID を解決する。" },
                        ["nodeGeneration"] = new JsonObject { ["type"] = "string", ["maxLength"] = 64, ["description"] = "node_generation from the grouped-cycle or mapping response; stale generations are rejected. / グループ化循環またはマッピング応答の node_generation。古い世代は拒否する。" },
                        ["mappingCursor"] = new JsonObject { ["type"] = "string", ["maxLength"] = 256, ["description"] = "next_mapping_cursor, or next_file_cursor with cycleNode and nodeGeneration. / next_mapping_cursor、または cycleNode と nodeGeneration を伴う next_file_cursor。" },
                        ["maxBytes"] = new JsonObject { ["type"] = "integer", ["minimum"] = 1, ["maximum"] = DependencyCycleNavigation.DefaultMaxBytes, ["description"] = "nodeMappings only: UTF-8 structuredContent budget, including a CLI newline for parity; default/cap 65536. / nodeMappings 専用。CLI との互換性のため改行を含めた structuredContent の UTF-8 上限。既定・最大65536。" },
    ...
    private static JsonObject[] CreateAuditAndFeedbackTools()
    {
        ...
        [
            ...
            CreateToolDefinition(
                "suggest_improvement",
                "Store cdidx suggestions or error reports locally; may publish an upstream GitHub issue when CDIDX_GITHUB_TOKEN is configured. "
                ...
                + "/ cdidxへの改善提案またはエラー報告をローカル保存し、CDIDX_GITHUB_TOKEN 設定時には上流の GitHub Issue として公開することがある。"
