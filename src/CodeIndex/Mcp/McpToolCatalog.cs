...
public partial class McpServer
{
    private static JsonArray CreateToolCatalog()
    {
        var tools = new JsonArray
        {
            ...
            CreateToolDefinition(
                "validate",
                "Report encoding issues found during indexing: U+FFFD replacement chars, BOM markers, null bytes, mixed/CR-only line endings, UTF-16 BOM detection, likely non-UTF8 encodings. replacement_char rows include origin/severity metadata so agents can separate source literals from decoder replacements. Page metadata includes authoritative totals, `result_stable_at`, and an opaque generation-bound `next_cursor`; pass it back unchanged with the same filters, format, and limit. / インデックス時に検出したエンコーディング問題を報告。replacement_char 行は source literal と decoder replacement を分ける origin/severity metadata を含む。ページ metadata は authoritative total、`result_stable_at`、generation-bound な opaque `next_cursor` を含む。同じ filter / format / limit で cursor を変更せず渡す。",
                new JsonObject
                {
                    ["type"] = "object",
                    ["properties"] = new JsonObject
