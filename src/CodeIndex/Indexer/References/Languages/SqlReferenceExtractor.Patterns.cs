...
internal static partial class SqlReferenceExtractor
{
    ...
    private static readonly Regex TargetReferenceRegex = new(
        $@"(?<![\w$])(?:INSERT(?:\s+{TopTargetModifierPattern})?\s+(?:INTO\s+|(?!OR\b|IGNORE\b|OVERWRITE\b|LOW_PRIORITY\b|DELAYED\b|HIGH_PRIORITY\b)){QualifiedIdentifierNoCapturePattern}|BULK\s+INSERT\s+{QualifiedIdentifierNoCapturePattern}|UPDATE\b(?:\s+{TopTargetModifierPattern})\s+{QualifiedIdentifierNoCapturePattern}|UPDATE\b(?:\s+ONLY\b)*\s+{QualifiedIdentifierNoCapturePattern}|MERGE\b(?:\s+{TopTargetModifierPattern})?(?:\s+INTO)?\s+{QualifiedIdentifierNoCapturePattern}|DELETE\b(?:\s+{TopTargetModifierPattern})?\s+FROM(?:\s+ONLY\b)?\s+{QualifiedIdentifierNoCapturePattern}|ALTER\s+TABLE\s+{QualifiedIdentifierNoCapturePattern})",
    ...
    private static readonly Regex CreateTempTableRegex = new(
        ...
        RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
    private static readonly Regex UsingKeywordRegex = new(@"(?<![\w$])USING\b", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
