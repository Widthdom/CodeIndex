...
- **Changelog validation skips an unused SDK install** — Its net8-only tool job now installs only the pinned .NET 8 SDK instead of also provisioning .NET 9.
...
- **changelog検証で未使用SDKの導入を省きました** — net8専用toolのjobでは、.NET 9も用意せずpin済み.NET 8 SDKだけを導入します。
