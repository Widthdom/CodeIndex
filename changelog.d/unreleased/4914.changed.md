...
- **部分 upgrade が旧 C# family row のまま reference identity v7 を公開しなくなりました (#4914)** — full・scoped・MCP・fold-backfill の graph refresh は authoritative な C# partial-family metadata を再 stamp できるまで reference-identity contract を stale のまま維持し、未変更の v7 より前の file-local symbol が新しい reference candidate 解決へ混入することを防ぎます。
