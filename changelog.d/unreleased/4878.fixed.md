...
- **count mode の出力形式を一貫して検証するようになりました (#4878)** — `--count` と `--format count` が同じ output-mode 検証を使い、明示的な JSON array / NDJSON stream や非互換 format は要求した形式を黙って置き換えず usage error となり、反復した `--format` は既存の右端優先規則を維持します。
