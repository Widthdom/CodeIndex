...
- **Production SQL boundaries reuse indexed workspaces** — Line-end comment and `USING` / `MERGE` variants now share one production CLI indexing subprocess per family while path filters preserve exact counts, kinds, qualified names, and negatives.
...
- **production SQL boundaryでindexed workspaceを再利用します** — line-end commentと`USING` / `MERGE` variantはfamilyごとにproduction CLI indexing subprocessを1回だけ共有し、path filterでexact count、kind、qualified name、negativeを維持します。
