...
- **Production SQL boundaries reuse indexed workspaces** — Line-end comment variants now share one production CLI indexing subprocess while path filters preserve each source's exact reference counts and kinds.
...
- **production SQL boundaryでindexed workspaceを再利用します** — line-end comment variantはproduction CLI indexing subprocessを1回だけ共有し、path filterでsourceごとのexact reference count / kindを維持します。
