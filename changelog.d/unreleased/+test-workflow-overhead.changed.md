...
- **Production SQL boundaries reuse indexed workspaces** — Line-end comment, `USING` / `MERGE`, and semicolonless temporary-table variants now share one production CLI indexing subprocess per family while path filters preserve exact contracts.
...
- **production SQL boundaryでindexed workspaceを再利用します** — line-end comment、`USING` / `MERGE`、semicolonless temporary-table variantはfamilyごとにproduction CLI indexing subprocessを1回だけ共有し、path filterでexact contractを維持します。
