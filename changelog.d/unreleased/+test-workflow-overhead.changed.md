...
- **Production query boundaries reuse indexed workspaces** — Related C# order-by comma, ternary-operator, and throw-expression cases now share one production CLI indexing subprocess per syntax family while retaining distinct assertions.
...
- **production query boundaryでindexed workspaceを再利用します** — 関連するC# order-by comma、ternary operator、throw-expression caseは個別の構文assertionを保ちながら、構文familyごとにproduction CLI indexing subprocessを1回だけ共有します。
